...
use DevTheorem\Handlebars\Handlebars as HandlebarsEngine;
...
class Handlebars
{
    ...
    private function runtime_options(): array
    {
        $options = [
            ...
            'partialResolver' => fn (string $name): ?Closure => $this->resolve_partial($name),
        ];
    ...
    private function compilation_cache_key(): string
    {
        ...
        $fingerprint = [
            'package' => $this->package_version(),
            'cache_version' => self::CACHE_VERSION,
            'compat' => $this->options->compat,
            'known_helpers' => $known_helpers,
            'known_helpers_only' => $this->options->knownHelpersOnly,
            'no_escape' => $this->options->noEscape,
            'strict' => $this->options->strict,
            'assume_objects' => $this->options->assumeObjects,
            'prevent_indent' => $this->options->preventIndent,
            'ignore_standalone' => $this->options->ignoreStandalone,
            'explicit_partial_context' => $this->options->explicitPartialContext,
        ];
    ...
    private function package_version(): string
    {
        if (class_exists(InstalledVersions::class) && InstalledVersions::isInstalled('devtheorem/php-handlebars')) {
            return (InstalledVersions::getPrettyVersion('devtheorem/php-handlebars') ?? 'unknown')
                . ':' . (InstalledVersions::getReference('devtheorem/php-handlebars') ?? 'unknown')
                . ':' . self::CACHE_VERSION;
        }
    ...
    private function normalize_value(mixed $value): mixed
    {
        ...
        if (is_object($value)) {
            return new HandlebarsContextValue($value, $this->normalize_value(...));
        }
    ...
    {
        if ($value instanceof HandlebarsContextValue) {
            return $value->value();
        }
