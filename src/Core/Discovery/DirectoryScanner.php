...
final class DirectoryScanner
{
     ...
     */
    public function __construct(private readonly array $discoveries)
    {
    }
    /**
     ...
     */
    public function scan(\Picowind\Core\Discovery\DiscoveryLocation $location, string $path, array $skipFiles = []): void
    {
        $input = realpath($path);
        ...
        foreach ($this->discoveries as $discovery) {
            if ($discovery instanceof \Picowind\Core\Discovery\DiscoversPath) {
