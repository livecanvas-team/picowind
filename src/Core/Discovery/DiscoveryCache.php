...
use PicowindDeps\Symfony\Component\Cache\Adapter\FilesystemAdapter;
use Throwable;
final class DiscoveryCache
{
    private FilesystemAdapter $cache;
    public function __construct(private readonly \Picowind\Core\Discovery\DiscoveryCacheStrategy $discoveryCacheStrategy)
    {
        $uploadDir = wp_get_upload_dir();
        $cacheDir = $uploadDir['basedir'] . '/picowind/cache/';
        wp_mkdir_p($cacheDir);
        $this->cache = new FilesystemAdapter(namespace: 'discovery', defaultLifetime: 0, directory: $cacheDir);
    }
    public function isEnabled(): bool
    {
        return $this->discoveryCacheStrategy !== \Picowind\Core\Discovery\DiscoveryCacheStrategy::DISABLED;
    }
    /**
     * @param array<Discovery> $discoveries
     */
    public function store(\Picowind\Core\Discovery\DiscoveryLocation $discoveryLocation, array $discoveries): void
    {
        if (!$this->isEnabled()) {
            return;
        }
     ...
     */
    public function restore(\Picowind\Core\Discovery\DiscoveryLocation $discoveryLocation): ?array
    {
        if (!$this->isEnabled()) {
            return null;
        }
    ...
    {
        return \false;
        return $this->discoveryCacheStrategy === \Picowind\Core\Discovery\DiscoveryCacheStrategy::PARTIAL && defined('WP_DEBUG') && \WP_DEBUG;
    }
    private function getCacheKey(\Picowind\Core\Discovery\DiscoveryLocation $discoveryLocation): string
    {
     ...
     */
    private function createManifest(\Picowind\Core\Discovery\DiscoveryLocation $discoveryLocation): array
    {
        $files = [];
     ...
     */
    private function isManifestStale(\Picowind\Core\Discovery\DiscoveryLocation $discoveryLocation, array $cachedManifest): bool
    {
        if (!is_dir($discoveryLocation->path)) {
            return \false;
        }
    ...
    private function logError(string $message, Throwable $exception, ?\Picowind\Core\Discovery\DiscoveryLocation $discoveryLocation = null, ?string $cacheKey = null): void
    {
