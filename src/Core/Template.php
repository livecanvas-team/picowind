...
class Template
{
    public function __construct(private readonly RenderTwig $renderTwig, private readonly RenderBlade $renderBlade, private readonly RenderLatte $renderLatte, private readonly RenderHandlebars $renderHandlebars)
    {
    }
    /**
     ...
     */
    public function render_string(string $template_string, array $context = [], string $engine = 'twig', ?bool $print = \true): ?string
    {
     ...
     * @return void|string Rendered output if $print is false, otherwise void
     */
    public function render_template(string|array $paths, array $context = [], ?string $engine = null, ?bool $print = \true): ?string
    {
        ...
        if (is_array($paths)) {
            ...
            throw new UnsupportedRenderEngineException('mixed', "\nNo template could be rendered. Tried:\n" . implode("\n", array_map(fn($err, $i) => "#{$i} {$err}", $errors, array_keys($errors))) . "\n");
        }
        if (null === $engine) {
