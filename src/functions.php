 ...
 */
function render($paths, array $context = [], ?string $engine = null, ?bool $print = \true, ?bool $silent = \false)
{
    ...
    try {
        ...
        if (!$silent) {
            error_log(sprintf("[Picowind Render Error]\nPaths: %s\nEngine: %s\nMessage: %s\nFile: %s:%d\nTrace:\n%s", is_array($paths) ? implode(', ', $paths) : $paths, $engine ?? 'auto-detect', $e->getMessage(), $e->getFile(), $e->getLine(), $e->getTraceAsString()));
        }
        // Engine and paths info.
        $failMessage = sprintf("[Picowind Render Failed]\nEngine: %s\nPaths: %s\n", $engine ?? 'auto-detect', is_array($paths) ? implode(', ', $paths) : $paths);
        if ($print && !$silent) {
            echo $failMessage;
        }
        return null;
 ...
 * @return void|string The rendered template output if $print is false, otherwise void.
 */
function render_string(string $template_string, array $context = [], string $engine = 'twig', ?bool $print = \true)
{
    $theme = \Picowind\Theme::get_instance();
    $container = $theme->container();
    /** @var Template */
    ...
    try {
        ...
        error_log(sprintf("[Picowind Render String Error]\nEngine: %s\nTemplate: %s\nMessage: %s\nFile: %s:%d\nTrace:\n%s", $engine, substr($template_string, 0, 200), $e->getMessage(), $e->getFile(), $e->getLine(), $e->getTraceAsString()));
        $failMessage = sprintf("[Picowind Render String Failed]\nEngine: %s\n", $engine);
        if ($print) {
            echo $failMessage;
            return null;
        }
