...
class Menu
{
    ...
    private function build_menu_tree(array $items): array
    {
        ...
        foreach ($items as $item) {
            $menu_items_by_id[$item->id] = ['id' => $item->id, 'title' => $item->title, 'url' => $item->url, 'target' => $item->target ?: null, 'classes' => implode(' ', $item->classes ?: []), 'current' => $item->current, 'current_item_ancestor' => $item->current_item_ancestor, 'current_item_parent' => $item->current_item_parent, 'attr_title' => $item->attr_title ?: null, 'description' => $item->description ?: null, 'parent_id' => (int) $item->menu_item_parent, 'children' => []];
        }
        ...
        foreach ($menu_items_by_id as $id => $item) {
            ...
            } else if (isset($menu_items_by_id[$item['parent_id']])) {
    ...
    public function add_to_context(array $context): array
    {
        ...
        foreach ($locations as $location => $menu_id) {
            ...
            if ($items) {
                ...
                $formatted_items = [];
                foreach ($items as $item) {
                    $formatted_items[] = (object) ['id' => $item->ID, 'title' => $item->title, 'url' => $item->url, 'target' => $item->target ?: '_self', 'classes' => $item->classes, 'current' => $item->current, 'current_item_ancestor' => $item->current_item_ancestor, 'current_item_parent' => $item->current_item_parent, 'attr_title' => $item->attr_title, 'description' => $item->description, 'menu_item_parent' => $item->menu_item_parent];
                }
