...
class Shortcode
{
    ...
    {
        if (!apply_filters('f!picowind/supports/shortcode:register', \true)) {
            return;
        }
    ...
    {
        $atts = shortcode_atts(['template' => null], $atts, 'twig');
    ...
    {
        $atts = shortcode_atts(['template' => null], $atts, 'blade');
        $globalContext = context();
        if ($atts['template']) {
    ...
    {
        $atts = shortcode_atts(['template' => null], $atts, 'latte');
        $globalContext = context();
        if ($atts['template']) {
