...
use PicowindDeps\Blockstudio\Build;
...
class Blockstudio
{
    ...
    {
        if (isset($block_json['blockstudio']) && (\true !== $block_json['blockstudio'] || 1 !== $block_json['blockstudio'])) {
            return \false;
        }
    ...
    public function block_render($value, $block, $isEditor, $isPreview)
    {
        ...
        $rendered = render($blockPath, ['a' => $data['attributes'], 'attributes' => $data['attributes'], 'b' => $data['block'], 'block' => $data['block'], 'c' => $data['context'], 'context' => $data['context'], 'isPreview' => $isPreview, 'isEditor' => $isEditor], null, \false);
        return $rendered;
    }
}
