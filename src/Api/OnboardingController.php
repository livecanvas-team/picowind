...
use PicowindDeps\Symfony\Component\Filesystem\Filesystem;
use WP_REST_Request;
...
final class OnboardingController
{
     ...
     */
    #[Route(path: '/dashboard', methods: 'GET', permission_callback: 'manage_options')]
    public function getDashboard(): WP_REST_Response
    {
        try {
            ...
            $activePlugins = array_filter($plugins, static fn(array $plugin): bool => (bool) ($plugin['active'] ?? \false));
            $installedThemes = array_filter($themes, static fn(array $theme): bool => (bool) ($theme['installed'] ?? \false));
            $totalSetupItems = count($plugins) + 1;
            $readySetupItems = count($activePlugins) + (!empty($childTheme) ? 1 : 0);
            $setupProgress = $totalSetupItems > 0 ? (int) round($readySetupItems / $totalSetupItems * 100) : 100;
            return new WP_REST_Response(['success' => \true, 'data' => ['status' => ['completed' => (bool) Config::get('onboarding.completed', \false), 'hasChildTheme' => !empty($childTheme), 'childTheme' => $childTheme], 'themes' => array_values($themes), 'plugins' => array_values($plugins), 'summary' => ['setupProgress' => $setupProgress, 'readyItems' => $readySetupItems, 'totalItems' => $totalSetupItems, 'installedThemes' => count($installedThemes), 'availableThemes' => count($themes), 'activePlugins' => count($activePlugins), 'recommendedPlugins' => count($plugins)]]], 200);
        } catch (Exception $e) {
     ...
     */
    #[Route(path: '/status', methods: 'GET', permission_callback: 'manage_options')]
    public function getStatus(WP_REST_Request $request): WP_REST_Response
    {
        try {
            $isCompleted = Config::get('onboarding.completed', \false);
            $childTheme = $this->getActiveChildTheme();
            return new WP_REST_Response(['success' => \true, 'data' => ['completed' => $isCompleted, 'hasChildTheme' => !empty($childTheme), 'childTheme' => $childTheme]], 200);
        } catch (Exception $e) {
            return new WP_REST_Response(['success' => \false, 'message' => $e->getMessage()], 500);
        }
    }
     ...
     */
    #[Route(path: '/themes', methods: 'GET', permission_callback: 'manage_options')]
    public function getAvailableThemes(WP_REST_Request $request): WP_REST_Response
    {
        try {
            $themes = $this->getBundledThemes();
            return new WP_REST_Response(['success' => \true, 'data' => $themes], 200);
        } catch (Exception $e) {
            return new WP_REST_Response(['success' => \false, 'message' => $e->getMessage()], 500);
        }
    }
     ...
     */
    #[Route(path: '/plugins', methods: 'GET', permission_callback: 'manage_options')]
    public function getRecommendedPlugins(WP_REST_Request $request): WP_REST_Response
    {
        try {
            $plugins = $this->getRecommendedPluginsWithStatus();
            return new WP_REST_Response(['success' => \true, 'data' => $plugins], 200);
        } catch (Exception $e) {
            return new WP_REST_Response(['success' => \false, 'message' => $e->getMessage()], 500);
        }
    }
     ...
     */
    #[Route(path: '/install-theme', methods: 'POST', permission_callback: 'manage_options', args: ['themeId' => ['required' => \true, 'type' => 'string', 'description' => 'The ID of the theme to install', 'sanitize_callback' => 'sanitize_text_field']])]
    public function installTheme(WP_REST_Request $request): WP_REST_Response
    {
        ...
        try {
            ...
            if (!isset($bundledThemes[$themeId])) {
                return new WP_REST_Response(['success' => \false, 'message' => 'Theme not found in bundled catalog'], 404);
            }
            $themeData = $bundledThemes[$themeId];
            $installedTheme = $this->findInstalledTheme(wp_get_themes(), $themeData);
            if ($installedTheme) {
                return new WP_REST_Response(['success' => \true, 'message' => 'Theme is already installed', 'data' => ['installed' => \true, 'themeSlug' => $installedTheme->get_stylesheet(), 'theme' => $this->stripBundledThemePath($themeData)]], 200);
            }
            $installResult = $this->installBundledTheme($themeData);
            if (is_wp_error($installResult)) {
                return new WP_REST_Response(['success' => \false, 'message' => $installResult->get_error_message()], 500);
            }
            // Store selected theme in config
            Config::set('onboarding.selectedTheme', $themeId);
            return new WP_REST_Response(['success' => \true, 'message' => 'Theme installed successfully', 'data' => ['installed' => \true, 'themeSlug' => $installResult['themeSlug'], 'theme' => $this->stripBundledThemePath($themeData)]], 200);
        } catch (Exception $e) {
            return new WP_REST_Response(['success' => \false, 'message' => $e->getMessage()], 500);
        }
    }
     ...
     */
    #[Route(path: '/activate-theme', methods: 'POST', permission_callback: 'manage_options', args: ['themeSlug' => ['required' => \true, 'type' => 'string', 'description' => 'The theme slug to activate', 'sanitize_callback' => 'sanitize_text_field']])]
    public function activateTheme(WP_REST_Request $request): WP_REST_Response
    {
        ...
        try {
            ...
            if (is_wp_error($activationResult)) {
                return new WP_REST_Response(['success' => \false, 'message' => $activationResult->get_error_message()], 500);
            }
            Config::set('onboarding.selectedTheme', $themeSlug);
            return new WP_REST_Response(['success' => \true, 'message' => 'Theme activated successfully', 'data' => ['activated' => \true, 'themeSlug' => $themeSlug, 'activeTheme' => $this->getActiveChildTheme()]], 200);
        } catch (Exception $e) {
            return new WP_REST_Response(['success' => \false, 'message' => $e->getMessage()], 500);
        }
    }
     ...
     */
    #[Route(path: '/install-plugin', methods: 'POST', permission_callback: 'manage_options', args: ['slug' => ['required' => \true, 'type' => 'string', 'description' => 'The plugin slug to install', 'sanitize_callback' => 'sanitize_text_field']])]
    public function installPlugin(WP_REST_Request $request): WP_REST_Response
    {
        ...
        try {
            ...
            if (!$plugin) {
                return new WP_REST_Response(['success' => \false, 'message' => 'Plugin is not available for installation'], 404);
            }
            ...
            if ($existingFile) {
                return new WP_REST_Response(['success' => \true, 'message' => 'Plugin is already installed', 'data' => ['installed' => \true, 'active' => $this->isPluginActive($existingFile)]], 200);
            }
            require_once \ABSPATH . 'wp-admin/includes/class-wp-upgrader.php';
            require_once \ABSPATH . 'wp-admin/includes/plugin-install.php';
            require_once \ABSPATH . 'wp-admin/includes/file.php';
            $pluginInfo = plugins_api('plugin_information', ['slug' => $slug, 'fields' => ['sections' => \false, 'versions' => \false]]);
            if (is_wp_error($pluginInfo)) {
                return new WP_REST_Response(['success' => \false, 'message' => $pluginInfo->get_error_message()], 500);
            }
            ...
            if (!$result) {
                return new WP_REST_Response(['success' => \false, 'message' => 'Plugin installation failed'], 500);
            }
            $pluginFile = $this->getPluginFileBySlug($slug);
            if (!$pluginFile) {
                return new WP_REST_Response(['success' => \false, 'message' => 'Plugin installation completed, but plugin file could not be found'], 500);
            }
            return new WP_REST_Response(['success' => \true, 'message' => 'Plugin installed successfully', 'data' => ['installed' => \true, 'active' => $this->isPluginActive($pluginFile)]], 200);
        } catch (Exception $e) {
            return new WP_REST_Response(['success' => \false, 'message' => $e->getMessage()], 500);
        }
    }
     ...
     */
    #[Route(path: '/activate-plugin', methods: 'POST', permission_callback: 'manage_options', args: ['slug' => ['required' => \true, 'type' => 'string', 'description' => 'The plugin slug to activate', 'sanitize_callback' => 'sanitize_text_field']])]
    public function activatePlugin(WP_REST_Request $request): WP_REST_Response
    {
        ...
        try {
            ...
            $plugin = $catalog[$slug] ?? null;
            if (!$plugin) {
                return new WP_REST_Response(['success' => \false, 'message' => 'Plugin is not available for activation'], 404);
            }
            if (!$this->canInstallRecommendedPlugin($plugin)) {
                return new WP_REST_Response(['success' => \false, 'message' => 'Plugin is not available for activation'], 404);
            }
            $pluginFile = $this->getPluginFileBySlug($slug);
            if (!$pluginFile) {
                return new WP_REST_Response(['success' => \false, 'message' => 'Plugin is not installed'], 404);
            }
            if ($this->isPluginActive($pluginFile)) {
                return new WP_REST_Response(['success' => \true, 'message' => 'Plugin is already active', 'data' => ['installed' => \true, 'active' => \true]], 200);
            }
            ...
            return new WP_REST_Response(['success' => \true, 'message' => 'Plugin activated successfully', 'data' => ['installed' => \true, 'active' => \true]], 200);
        } catch (Exception $e) {
            return new WP_REST_Response(['success' => \false, 'message' => $e->getMessage()], 500);
        }
    }
    ...
    private function activateThemeSlug(string $themeSlug)
    {
        ...
        return ['success' => \true, 'theme' => $themeSlug];
    }
    /**
     * Mark onboarding as completed
     */
    #[Route(path: '/complete', methods: 'POST', permission_callback: 'manage_options')]
    public function complete(): WP_REST_Response
    {
        try {
            Config::set('onboarding.completed', \true);
            Config::set('onboarding.completedAt', current_time('mysql'));
            return new WP_REST_Response(['success' => \true, 'message' => 'Onboarding completed successfully'], 200);
        } catch (Exception $e) {
            return new WP_REST_Response(['success' => \false, 'message' => $e->getMessage()], 500);
        }
    }
     ...
     */
    #[Route(path: '/reset', methods: 'POST', permission_callback: 'manage_options')]
    public function reset(): WP_REST_Response
    {
        try {
            ...
            return new WP_REST_Response(['success' => \true, 'message' => 'Onboarding reset successfully'], 200);
        } catch (Exception $e) {
            return new WP_REST_Response(['success' => \false, 'message' => $e->getMessage()], 500);
        }
    }
    ...
    private function getActiveChildTheme(): ?array
    {
        ...
        if ($currentTheme->parent()) {
            return ['name' => $currentTheme->get('Name'), 'slug' => $currentTheme->get_stylesheet(), 'version' => $currentTheme->get('Version'), 'parent' => $currentTheme->parent()->get('Name')];
        }
        return null;
    }
    /**
    ...
    private function getBundledThemesCatalog(): array
    {
        ...
        foreach ($entries as $entry) {
            ...
            $data = get_file_data($stylePath, ['Name' => 'Theme Name', 'ThemeURI' => 'Theme URI', 'Description' => 'Description', 'Author' => 'Author', 'Version' => 'Version', 'Template' => 'Template', 'TextDomain' => 'Text Domain', 'Tags' => 'Tags'], 'theme');
            $themes[$entry] = ['id' => $entry, 'path' => $themePath, 'name' => $data['Name'] ?: $entry, 'description' => $data['Description'] ?? '', 'version' => $data['Version'] ?? '', 'author' => $data['Author'] ?? '', 'themeUri' => $data['ThemeURI'] ?? '', 'template' => $data['Template'] ?? '', 'textDomain' => $data['TextDomain'] ?? '', 'tags' => $this->parseThemeTags($data['Tags'] ?? '')];
        }
        return $themes;
    }
    /**
    ...
    {
        $catalog = ['windpress' => ['id' => 'windpress', 'name' => 'WindPress', 'slug' => 'windpress', 'source' => 'wporg', 'url' => 'https://wordpress.org/plugins/windpress/', 'description' => 'Tailwind CSS integration for WordPress.'], 'yabe-webfont' => ['id' => 'yabe-webfont', 'name' => 'Yabe Webfont', 'slug' => 'yabe-webfont', 'source' => 'wporg', 'url' => 'https://wordpress.org/plugins/yabe-webfont/', 'description' => 'Use custom fonts, Google Fonts, or Adobe Fonts.'], 'jooosi-icon' => ['id' => 'jooosi-icon', 'name' => 'Jooosi Icon', 'slug' => 'jooosi-icon', 'source' => 'wporg', 'url' => 'https://wordpress.org/plugins/jooosi-icon/', 'description' => 'Modern SVG icon library for WordPress.'], 'livecanvas' => ['id' => 'livecanvas', 'name' => 'LiveCanvas', 'slug' => 'livecanvas', 'source' => 'external', 'url' => 'https://livecanvas.com/', 'description' => 'Professional page builder for WordPress with front-end editing.']];
    ...
    private function getRecommendedPluginsWithStatus(): array
    {
        ...
        foreach ($catalog as $plugin) {
            ...
            $plugins[] = array_merge($plugin, ['installed' => $pluginFile !== null, 'active' => $pluginFile ? $this->isPluginActive($pluginFile) : \false]);
        }
    ...
    private function getBundledThemes(): array
    {
        ...
        foreach ($bundledThemes as $theme) {
            ...
            $themes[] = array_merge($this->stripBundledThemePath($theme), ['installed' => (bool) $installedTheme, 'active' => $installedTheme ? $installedSlug === $activeSlug : \false, 'installedSlug' => $installedSlug]);
        }
    ...
    private function installBundledTheme(array $bundledTheme)
    {
        ...
        return ['success' => \true, 'themeSlug' => $themeSlug];
    }
    /**
