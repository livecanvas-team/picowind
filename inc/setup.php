...
add_action( 'after_setup_theme', 'picowind_setup' );

if ( ! function_exists( 'picowind_setup' ) ) {
	/**
	 ...
	 */
	function picowind_setup() {
		/*
		 * Make theme available for translation.
		 * Translations can be filed in the /languages/ directory.
		 * If you're building a theme based on this one, use a find and replace
		 * to change 'picowind' to the name of your theme in all the template files
		 */
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'picowind_custom_background_args',
				array(
		...
		*/
		...
		picowind_setup_theme_default_settings();
...
add_filter( 'excerpt_more', 'picowind_custom_excerpt_more' );

if ( ! function_exists( 'picowind_custom_excerpt_more' ) ) {
	/**
	 ...
	 * @return string
	 */
	function picowind_custom_excerpt_more( $more ) {
...
add_filter( 'wp_trim_excerpt', 'picowind_all_excerpts_get_more_link' );

if ( ! function_exists( 'picowind_all_excerpts_get_more_link' ) ) {
	/**
	 ...
	 * @return string
	 */
	function picowind_all_excerpts_get_more_link( $post_excerpt ) {
		...
		if ( ! is_admin() OR ( isset($_POST['action']) && $_POST['action'] == 'lc_process_dynamic_templating_shortcode') ) {
			$post_excerpt = $post_excerpt . '...<p class="text-end"><a class="btn btn-outline-secondary picowind-read-more-link mt-3" href="' . esc_url( get_permalink( get_the_ID() ) ) . '">' . __(
				'Read More...',
				'picowind'
			) . '</a></p>';
		}
