if (0) (function($) {
	...
	function ps_save_css_bundle(){
		...
		formdata.append("nonce", picowind_ajax_obj.nonce);
		formdata.append("action", "picowind_save_css_bundle");
		formdata.append("css", theCss);
		fetch(picowind_ajax_obj.ajax_url, {
			method: "POST",
...
@font-face {
    ...
    function ps_update_fonts_import_code_snippet_from_object_fields() {
        ...
        $("#_customize-input-picowind_fonts_header_code").val(html_code).change();
	...
	$(document).ready(function() {
        ...
        var headerEditor = wp.codeEditor.initialize($('#_customize-input-picowind_header_code')).codemirror;
        var footerEditor = wp.codeEditor.initialize($('#_customize-input-picowind_footer_code')).codemirror;
        ...
        function updateCustomizerOriginalFields() {
            ...
            wp.customize('picowind_header_code').set(headerContent);
            wp.customize('picowind_footer_code').set(footerContent);
        }
		...
		*/
        ...
        if ( ($("#_customize-input-body_font_object").val().length > 10) && !$("#_customize-input-body_font_object").val().includes('cssImport')) {
            ...
            if (confirm("Your font import settings seem obsolete. Do you want to try to auto-update them? ")) {
                ...
                $("#_customize-input-picowind_fonts_header_code").val("");
                ps_update_font_objects_and_import_code(); 
            } 
        }
		...
		$("body").on("click",".cs-toggle-csseditor-position",function(e){
			...
			$('#customize-control-custom_css').toggleClass('picowind-maximize-editor');
		});
		...
		$("body").on("click", ".download-scss-vars", function (e) {
			...
			a.href = url;
			a.download = 'picowind_bs_variables.json';
			a.click();
