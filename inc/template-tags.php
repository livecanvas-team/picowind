...
if(!function_exists('picowind_site_info')):
	function picowind_site_info(){
		$footer_text_setting = get_theme_mod("picowind_footer_text");
		?>
			<div class="site-info small">
				<?php if (strlen($footer_text_setting) > 0) echo $footer_text_setting; ?>
				<?php if (current_user_can("administrator") && strlen($footer_text_setting) <= 0) _e( 'You can edit this footer text using the WordPress Customizer.', 'picowind' ); ?>
			</div>
...
///SHARING BUTTONS ////
if(!function_exists('picowind_the_sharing_buttons')):
	function picowind_the_sharing_buttons(){

		global $post;
		$url_to_share=esc_attr(get_permalink($post->ID));
		?>
		<div class="picowind-sharing-buttons mt-4 mb-5" >
		
			<!-- Basic Share Links -->
			<span class="d-block mb-3 h4"><?php _e( 'Share this post', 'picowind' ); ?>: &nbsp; </span>
