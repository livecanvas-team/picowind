...
namespace PicowindDeps\Latte\Tools;
...
final class Linter
{
    public function __construct(private ?Latte\Engine $engine = null, private bool $debug = \false, private bool $strict = \false)
    {
    }
    public function scanDirectory(string $path): bool
    {
        ...
        foreach ($files as $file) {
            $file = (string) $file;
            echo preg_replace('~\.?[/\\\\]~A', '', $file), "\r";
            $errors += $this->lintLatte($file) ? 0 : 1;
            echo str_pad('...', strlen($file)), "\r";
            $counter++;
        }
    ...
    public function lintLatte(string $file): bool
    {
        ...
        try {
            $this->getEngine()->setLoader(new Latte\Loaders\StringLoader())->compile($s);
