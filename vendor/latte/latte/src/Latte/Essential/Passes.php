...
use PicowindDeps\Latte\Compiler\Nodes\Html\AttributeNode;
...
final class Passes
{
    ...
    public function customFunctionsPass(TemplateNode $node): void
    {
        ...
        (new NodeTraverser())->traverse($node, function (Node $node) use ($names) {
            if (($node instanceof Expression\FunctionCallNode || $node instanceof Expression\FunctionCallableNode) && $node->name instanceof Php\NameNode && $orig = $names[strtolower((string) $node->name)] ?? null) {
                if ((string) $node->name !== $orig) {
                    trigger_error("Case mismatch on function name '{$node->name}', correct name is '{$orig}'.", \E_USER_WARNING);
                }
                return new Expression\AuxiliaryNode(fn(PrintContext $context, ...$args) => '($this->global->fn->' . $orig . ')($this, ' . $context->implode($args) . ')', $node->args);
            }
        });
    }
    /**
    ...
    public function forbiddenVariablesPass(TemplateNode $node): void
    {
        ...
        (new NodeTraverser())->traverse($node, function (Node $node) use ($forbidden) {
            if ($node instanceof VariableNode && is_string($node->name) && (str_starts_with($node->name, 'ʟ_') || in_array($node->name, $forbidden, \true))) {
    ...
    public function scriptTagQuotesPass(TemplateNode $node): void
    {
        ...
        (new NodeTraverser())->traverse($node, function (Node $node) {
            if ($node instanceof ElementNode && $node->is('script') && HtmlHelpers::classifyScriptType((string) $node->getAttribute('type')) === ContentType::JavaScript) {
                $prev = null;
    ...
    public function checkUrlsPass(TemplateNode $node): void
    {
        ...
        (new NodeTraverser())->traverse($node, function (Node $node) use (&$elem) {
            if ($node instanceof ElementNode) {
                $elem = $node;
            } elseif ($node instanceof AttributeNode && $node->name instanceof TextNode && HtmlHelpers::isUrlAttribute($elem->name, $node->name->content)) {
                $attrValue = $node->value instanceof FragmentNode && $node->value->children ? $node->value->children[0] : $node->value;
                if ($attrValue instanceof PrintNode && ($modifier = $attrValue->modifier) && !$modifier->removeFilter('nocheck') && !$modifier->removeFilter('noCheck') && !$modifier->hasFilter('datastream') && !$modifier->hasFilter('dataStream')) {
