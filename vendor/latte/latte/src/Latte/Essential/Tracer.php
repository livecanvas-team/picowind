...
final class Tracer
{
    public static function throw(): void
    {
        ...
        $props = ['file' => $source['name'], 'line' => $source['line'] ?? 0, 'trace' => self::generateTrace($trace)];
            ...
            if ($object instanceof Template) {
                ...
                if (str_starts_with($method, 'block')) {
                    ...
                    if ($method !== 'renderBlock' || isset($item['args'][2])) {
                        // is not {block}
                        $res[] = ['function' => '{include ' . ($method === 'renderParentBlock' ? 'parent' : $item['args'][0]) . '}', 'file' => $object->getName(), 'line' => Helpers::mapCompiledToSource($item['file'], $item['line'])['line'] ?? 0, 'args' => self::filterParams($item['args'][1])];
                    }
                }
