...
use PicowindDeps\Latte\Runtime;
use PicowindDeps\Latte\RuntimeException;
use PicowindDeps\Nette;
...
final class CoreExtension extends Latte\Extension
{
    ...
    public function getFilters(): array
    {
        return [
            ...
            'capitalize' => extension_loaded('mbstring') ? [$this->filters, 'capitalize'] : fn() => throw new RuntimeException('Filter |capitalize requires mbstring extension.'),
            ...
            'firstLower' => extension_loaded('mbstring') ? [$this->filters, 'firstLower'] : fn() => throw new RuntimeException('Filter |firstLower requires mbstring extension.'),
            'firstUpper' => extension_loaded('mbstring') ? [$this->filters, 'firstUpper'] : fn() => throw new RuntimeException('Filter |firstUpper requires mbstring extension.'),
            ...
            'lower' => extension_loaded('mbstring') ? [$this->filters, 'lower'] : fn() => throw new RuntimeException('Filter |lower requires mbstring extension.'),
            ...
            'upper' => extension_loaded('mbstring') ? [$this->filters, 'upper'] : fn() => throw new RuntimeException('Filter |upper requires mbstring extension.'),
            'webalize' => class_exists(Nette\Utils\Strings::class) ? [Nette\Utils\Strings::class, 'webalize'] : fn() => throw new RuntimeException('Filter |webalize requires nette/utils package.'),
        ];
    }
    public function getFunctions(): array
    {
        return ['clamp' => [$this->filters, 'clamp'], 'divisibleBy' => [$this->filters, 'divisibleBy'], 'even' => [$this->filters, 'even'], 'first' => [$this->filters, 'first'], 'group' => [$this->filters, 'group'], 'last' => [$this->filters, 'last'], 'odd' => [$this->filters, 'odd'], 'slice' => [$this->filters, 'slice'], 'hasBlock' => fn(Runtime\Template $template, string $name): bool => $template->hasBlock($name), 'hasTemplate' => fn(Runtime\Template $template, string $name): bool => $this->hasTemplate($name, $template->getName())];
    }
    public function getPasses(): array
    {
        $passes = new Passes($this->engine);
        return ['internalVariables' => [$passes, 'forbiddenVariablesPass'], 'checkUrls' => [$passes, 'checkUrlsPass'], 'overwrittenVariables' => [Nodes\ForeachNode::class, 'overwrittenVariablesPass'], 'customFunctions' => [$passes, 'customFunctionsPass'], 'moveTemplatePrintToHead' => [Nodes\TemplatePrintNode::class, 'moveToHeadPass'], 'nElse' => [Nodes\NElseNode::class, 'processPass'], 'scriptTagQuotes' => [$passes, 'scriptTagQuotesPass']];
    }
    ...
    private function includeSplitter(Tag $tag, TemplateParser $parser): Nodes\IncludeBlockNode|Nodes\IncludeFileNode
    {
        ...
        return $block ? Nodes\IncludeBlockNode::create($tag, $parser) : Nodes\IncludeFileNode::create($tag);
    }
    /**
     * {syntax ...}
     */
