...
use PicowindDeps\Latte\Compiler\Nodes\NopNode;
...
class ForeachNode extends StatementNode
{
    ...
    public function print(PrintContext $context): string
    {
        ...
        if ($this->else) {
            $content .= $context->format('} if ($iterator->isEmpty()) %line { ', $this->elseLine) . $this->else->print($context);
        }
        if ($iterator) {
...
XX
, $this->expression, $this->printArgs($context), $this->position, $content);
        } else {
            return $context->format(<<<'XX'
...
, $this->expression, $this->printArgs($context), $this->position, $content);
    ...
    {
        return ($this->key ? $this->key->print($context) . ' => ' : '') . ($this->byRef ? '&' : '') . $this->value->print($context);
    }
    ...
    public static function overwrittenVariablesPass(TemplateNode $node): void
    {
        ...
        if ($vars) {
            array_unshift($node->head->children, new AuxiliaryNode(fn(PrintContext $context) => $context->format(<<<'XX'
