...
final class NAttrNode extends StatementNode
{
    ...
    {
        return $context->format('$ʟ_tmp = %node;
			echo %raw::attrs($ʟ_tmp, %dump) %line;', $this->args, self::class, $context->getEscaper()->getContentType() === Latte\ContentType::Xml, $this->position);
    }
    public static function attrs(mixed $attrs, bool $xml): string
    {
        $attrs = $attrs === [$attrs[0] ?? null] ? $attrs[0] : $attrs;
        // checks if the value is an array, e.g. n:attr="$attrs"
        if (!is_array($attrs)) {
            return '';
        }
        $res = '';
