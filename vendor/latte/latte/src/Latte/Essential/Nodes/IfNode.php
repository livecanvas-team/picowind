...
use PicowindDeps\Latte\Compiler\TagParser;
...
class IfNode extends StatementNode
{
    ...
    public static function create(Tag $tag, TemplateParser $parser): \Generator
    {
        ...
        $node->ifset = in_array($tag->name, ['ifset', 'elseifset'], \true);
        $node->capture = !$tag->isNAttribute() && $tag->name === 'if' && $tag->parser->isEnd();
        $node->position = $tag->position;
        if (!$node->capture) {
            $node->condition = $node->ifset ? self::buildCondition($tag->parser) : $tag->parser->parseExpression();
        }
    ...
    private static function buildCondition(TagParser $parser): ExpressionNode
    {
        ...
        do {
            ...
            $list[] = $block || $name instanceof StringNode ? new Expression\AuxiliaryNode(fn(PrintContext $context, ExpressionNode $name) => '$this->hasBlock(' . $context->ensureString($name, 'Block name') . ')', [$name]) : new Expression\IssetNode([$name]);
        } while ($parser->stream->tryConsume(','));
        return Expression\BinaryOpNode::nest('&&', ...$list);
    }
    public function print(PrintContext $context): string
    {
        return $this->capture ? $this->printCapturing($context) : $this->printCommon($context);
    }
    private function printCommon(PrintContext $context): string
    {
        if ($this->else) {
            return $context->format($this->else instanceof self ? "if (%node) %line { %node } else%node\n" : "if (%node) %line { %node } else %4.line { %3.node }\n", $this->condition, $this->position, $this->then, $this->else, $this->elseLine);
        }
        return $context->format("if (%node) %line { %node }\n", $this->condition, $this->position, $this->then);
    }
...
XX
, $this->position, $this->then, $this->elseLine, $this->else, $this->condition);
        }
        return $context->format(<<<'XX'
ob_start(fn() => '') %line;
try {
...
XX
, $this->position, $this->then, $this->condition);
    }
