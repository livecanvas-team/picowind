...
class BlockNode extends StatementNode
{
    ...
    public static function create(Tag $tag, TemplateParser $parser): \Generator
    {
        ...
        if (!$stream->is('|', Token::End)) {
            $layer = $tag->parser->tryConsumeTokenBeforeUnquotedString('local') ? Template::LayerLocal : $parser->blockLayer;
            $stream->tryConsume('#');
...
XX
, $this->position, $this->content, $context->getEscaper()->export(), $this->modifier);
    }
    private function printStatic(PrintContext $context): string
    {
        ...
        $this->block->content = $this->content->print($context);
        // must be compiled after is added
        return $context->format('$this->renderBlock(%raw, get_defined_vars()' . ($this->modifier->filters || $this->modifier->escape ? ', function ($s, $type) { $ʟ_fi = new LR\FilterInfo($type); return %modifyContent($s); }' : '') . ') %2.line;', $context->ensureString($this->block->name, 'Block name'), $this->modifier, $this->position);
    }
    private function printDynamic(PrintContext $context): string
    {
        ...
        return $context->format('$this->addBlock($ʟ_nm = %raw, %dump, [[$this, %dump]], %dump);
			$this->renderBlock($ʟ_nm, get_defined_vars()' . ($this->modifier->filters || $this->modifier->escape ? ', function ($s, $type) { $ʟ_fi = new LR\FilterInfo($type); return %modifyContent($s); }' : '') . ');', $context->ensureString($this->block->name, 'Block name'), $escaper->export(), $this->block->method, $this->block->layer, $this->modifier);
    }
