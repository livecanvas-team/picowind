...
class IncludeBlockNode extends StatementNode
{
    ...
    public static function create(Tag $tag, TemplateParser $parser): static
    {
        ...
        if ($node->parent && $node->modifier->filters) {
            ...
            $item = $tag->closestTag([BlockNode::class, DefineNode::class], fn($item) => $item->node?->block && !$item->node->block->isDynamic() && $item->node->block->name !== '');
            if (!$item) {
    ...
    public function print(PrintContext $context): string
    {
        ...
        $contentFilter = count($this->modifier->filters) > (int) $noEscape ? $context->format('function ($s, $type) { $ʟ_fi = new LR\FilterInfo($type); return %modifyContent($s); }', $this->modifier) : ($noEscape || $this->parent ? '' : PhpHelpers::dump($context->getEscaper()->export()));
        return $this->from ? $this->printBlockFrom($context, $contentFilter) : $this->printBlock($context, $contentFilter);
    }
    private function printBlock(PrintContext $context, string $contentFilter): string
    {
        ...
        return $context->format('$this->render' . ($this->parent ? 'ParentBlock' : 'Block') . '(%raw, %node? + ' . (isset($block) && !$block->parameters ? 'get_defined_vars()' : '[]') . '%raw) %line;', $context->ensureString($this->name, 'Block name'), $this->args, $contentFilter ? ", {$contentFilter}" : '', $this->position);
    }
    private function printBlockFrom(PrintContext $context, string $contentFilter): string
    {
        return $context->format('$this->createTemplate(%raw, %node? + $this->params, "include")->renderToContentType(%raw, %raw) %line;', $context->ensureString($this->from, 'Template name'), $this->args, $contentFilter, $context->ensureString($this->name, 'Block name'), $this->position);
    }
