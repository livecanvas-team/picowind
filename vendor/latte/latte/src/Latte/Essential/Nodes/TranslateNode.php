...
use PicowindDeps\Latte\Essential\TranslatorExtension;
...
class TranslateNode extends StatementNode
{
    ...
    public static function create(Tag $tag, ?callable $translator): \Generator
    {
        ...
        if ($text = NodeHelpers::toText($node->content)) {
            if ($translator && is_array($values = TranslatorExtension::toValue($args)) && is_string($translation = $translator($text, ...$values))) {
...
XX
, $context->getEscaper()->export(), $this->modifier, $this->content->content, $this->position);
        } else {
            return $context->format(<<<'XX'
ob_start(fn() => ''); try {
	%node
...
XX
, $this->content, $context->getEscaper()->export(), $this->modifier, $this->position);
        }
    }
