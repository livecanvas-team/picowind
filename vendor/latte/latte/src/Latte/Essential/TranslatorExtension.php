...
use PicowindDeps\Nette\Localization\Translator;
...
final class TranslatorExtension extends Latte\Extension
{
    ...
    private $translator;
    public function __construct(callable|Translator|null $translator, private ?string $key = null)
    {
    ...
    {
        return ['_' => [$this, 'parseTranslate'], 'translate' => fn(Tag $tag) => yield from Nodes\TranslateNode::create($tag, $this->key ? $this->translator : null)];
    }
    public function getFilters(): array
    {
        return ['translate' => fn(Latte\Runtime\FilterInfo $fi, ...$args) => $this->translator ? ($this->translator)(...$args) : $args[0]];
    }
    ...
    public function parseTranslate(Tag $tag): PrintNode
    {
        ...
        if ($this->translator && $this->key && ($expr = self::toValue($node->expression)) && is_array($values = self::toValue($args)) && is_string($translation = ($this->translator)($expr, ...$values))) {
    ...
    {
        try {
            return NodeHelpers::toValue($args, constants: \true);
