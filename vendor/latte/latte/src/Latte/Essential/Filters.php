...
namespace PicowindDeps\Latte\Essential;

use PicowindDeps\Latte;
use PicowindDeps\Latte\ContentType;
use PicowindDeps\Latte\Runtime\FilterInfo;
use PicowindDeps\Latte\Runtime\Html;
use Stringable;
...
final class Filters
{
    ...
    {
        return $info->contentType === ContentType::Html ? trim(self::spacelessHtml($s)) : trim(self::spacelessText($s));
    }
    /**
     * Replaces all repeated white spaces with a single space.
     */
     ...
     */
    public function localDate(string|int|\DateTimeInterface|null $value, ?string $format = null, ?string $date = null, ?string $time = null): ?string
    {
        if ($this->locale === null) {
        ...
        if ($format === null) {
            $xlt = ['' => \IntlDateFormatter::NONE, 'full' => \IntlDateFormatter::FULL, 'long' => \IntlDateFormatter::LONG, 'medium' => \IntlDateFormatter::MEDIUM, 'short' => \IntlDateFormatter::SHORT, 'relative-full' => \IntlDateFormatter::RELATIVE_FULL, 'relative-long' => \IntlDateFormatter::RELATIVE_LONG, 'relative-medium' => \IntlDateFormatter::RELATIVE_MEDIUM, 'relative-short' => \IntlDateFormatter::RELATIVE_SHORT];
            $time ??= '';
            $date ??= $time === '' ? 'long' : '';
        ...
        $res = preg_replace('~(\d\.) ~', "\$1 ", $res);
        return $res;
    }
    /**
     * Formats a number with grouped thousands and optionally decimal digits according to locale.
     */
    public function number(float $number, string|int $patternOrDecimals = 0, string $decimalSeparator = '.', string $thousandsSeparator = ','): string
    {
     ...
     */
    public static function replace(FilterInfo $info, string|array $subject, string|array $search, string|array|null $replace = null): string
    {
     ...
     */
    public static function truncate(string|Stringable|null $s, int $length, string $append = "…"): string
    {
        $s = (string) $s;
     ...
     * @return iterable<K, V>
     */
    public function sort(iterable $data, ?\Closure $comparison = null, string|int|\Closure|null $by = null, string|int|\Closure|bool $byKey = \false): iterable
    {
        if ($byKey !== \false) {
            if ($by !== null) {
                throw new \InvalidArgumentException('Filter |sort cannot use both $by and $byKey.');
            }
            $by = $byKey === \true ? null : $byKey;
        }
        if ($comparison) {
            ...
            $comparison = fn($a, $b) => is_string($a) && is_string($b) ? $collator->compare($a, $b) : $a <=> $b;
        }
        $comparison = match (\true) {
            $by === null => $comparison,
    ...
    public static function group(iterable $data, string|int|\Closure $by): iterable
    {
        ...
        foreach ($data as $k => $v) {
            ...
            if (!$groups || $prevKey !== $groupKey) {
                $index = array_search($groupKey, $keys, \true);
                if ($index === \false) {
                    $index = count($keys);
                    $keys[$index] = $groupKey;
                }
        ...
        return new AuxiliaryIterator(array_map(fn($key, $group) => [$key, new AuxiliaryIterator($group)], $keys, $groups));
    }
    /**
    ...
    {
        return is_array($value) ? $value ? $value[array_key_last($value)] : null : self::substring($value, -1);
    }
    /**
     * Extracts a slice of an array or string.
     */
    public static function slice(string|array $value, int $start, ?int $length = null, bool $preserveKeys = \false): string|array
    {
    ...
    {
        $s = $s instanceof Latte\Runtime\HtmlStringable ? Latte\Runtime\Filters::convertHtmlToText((string) $s) : (string) $s;
