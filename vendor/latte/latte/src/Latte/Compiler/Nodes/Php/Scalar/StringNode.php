...
{
    public function __construct(public string $value, public ?Position $position = null)
    {
    }
    public static function parse(string $str, Position $position): static
    {
        $str = $str[0] === "'" ? strtr(substr($str, 1, -1), ['\\\\' => '\\', "\\'" => "'"]) : PhpHelpers::decodeEscapeSequences(substr($str, 1, -1), '"');
