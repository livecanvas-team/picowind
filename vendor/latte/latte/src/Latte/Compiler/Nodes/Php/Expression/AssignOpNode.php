...
{
    private const Ops = ['+', '-', '*', '/', '.', '%', '&', '|', '^', '<<', '>>', '**', '??'];
    public function __construct(public ExpressionNode $var, public string $operator, public ExpressionNode $expr, public ?Position $position = null)
    {
        if (!in_array($this->operator, self::Ops, \true)) {
