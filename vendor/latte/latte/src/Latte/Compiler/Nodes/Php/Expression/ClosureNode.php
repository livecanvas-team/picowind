...
use PicowindDeps\Latte\Compiler\Nodes\Php\ClosureUseNode;
...
class ClosureNode extends ExpressionNode
{
    ...
    {
        (function (Php\ParameterNode ...$args) {
        })(...$params);
        (function (ClosureUseNode ...$args) {
        })(...$uses);
    }
    public function print(PrintContext $context): string
    {
        ...
        return $arrow ? 'fn' . ($this->byRef ? '&' : '') . '(' . $context->implode($this->params) . ')' . ($this->returnType !== null ? ': ' . $this->returnType->print($context) : '') . ' => ' . $this->expr->print($context) : 'function ' . ($this->byRef ? '&' : '') . '(' . $context->implode($this->params) . ')' . (!empty($this->uses) ? ' use (' . $context->implode($this->uses) . ')' : '') . ($this->returnType !== null ? ' : ' . $this->returnType->print($context) : '') . ($this->expr ? ' { return ' . $this->expr->print($context) . '; }' : ' {}');
    }
