...
{
    public function __construct(public float $value, public ?Position $position = null)
    {
    }
    public static function parse(string $str, Position $position): static
    {
        return strpbrk($str, '.eE') === \false ? new static((float) PhpHelpers::decodeNumber($str), $position) : new static((float) str_replace('_', '', $str), $position);
    }
