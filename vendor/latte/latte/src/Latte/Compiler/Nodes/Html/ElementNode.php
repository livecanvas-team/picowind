...
use PicowindDeps\Latte\Compiler\NodeHelpers;
...
class ElementNode extends AreaNode
{
    ...
    public function __construct(public string $name, public ?Position $position = null, public ?self $parent = null, public ?\stdClass $data = null, public string $contentType = ContentType::Html)
    {
    ...
    public function getAttribute(string $name): string|Node|bool|null
    {
        foreach ($this->attributes?->children ?? [] as $child) {
            if ($child instanceof AttributeNode && $child->name instanceof Nodes\TextNode && $this->matchesIdentifier($name, $child->name->content)) {
                return NodeHelpers::toText($child->value) ?? $child->value ?? \true;
            }
        }
        return null;
    }
    ...
    {
        return $this->contentType === ContentType::Html ? strcasecmp($a, $b) === 0 : $a === $b;
    }
    public function isRawText(): bool
    {
        return $this->contentType === ContentType::Html && ($this->is('script') || $this->is('style'));
    }
    public function print(PrintContext $context): string
    {
        ...
        if ($this->captureTagName || $this->variableName) {
            ...
            $endTag = var_export('</' . $this->name . '>', \true);
            $res = '';
        }
        $res .= $this->tagNode->print($context);
        // calls $this->printStartTag()
        $context->beginEscape()->enterHtmlText($this);
        $content = $this->content->print($context);
        $context->restoreEscape();
        $res .= $this->breakable ? 'try { ' . $content . ' } finally { echo ' . $endTag . '; } ' : $content . ' echo ' . $endTag . ';';
        return $res;
    }
    private function printStartTag(PrintContext $context): string
    {
        $context->beginEscape()->enterHtmlTag($this->name);
        $res = $this->variableName ? $context->format(<<<'XX'
...
echo %dump;
XX
, $this->contentType === ContentType::Html ? 'HtmlHelpers' : 'XmlHelpers', $this->variableName, $this->name, $this->endTagVar ? "{$this->endTagVar} = '</' . \$ʟ_tmp . '>' . {$this->endTagVar};" : '', $this->position, $this->attributes, $this->selfClosing ? '/>' : '>') : $context->format('%raw echo %dump; %node echo %dump;', $this->endTagVar ? $this->endTagVar . ' = ' . $context->encodeString("</{$this->name}>") . " . {$this->endTagVar};" : '', "<{$this->name}", $this->attributes, $this->selfClosing ? '/>' : '>');
        $context->restoreEscape();
        return $res;
    }
