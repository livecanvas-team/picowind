...
final class TemplateParser
{
    ...
    public function parseLatteStatement(?callable $resolver = null): ?Node
    {
        ...
        if ($this->stream->peek(1)->is(Token::Slash) || isset($this->tag, $this->lookFor[$this->tag]) && in_array($this->stream->peek(1)->text, $this->lookFor[$this->tag], \true)) {
            $this->lexer->popState();
            return null;
            // go back to previous parseLatteStatement()
        }
        $this->lexer->popState();
        $token = $this->stream->peek();
        ...
        if ($res instanceof \Generator) {
            ...
            if ($startTag->void) {
                ...
                while ($res->valid()) {
                    ...
                    if ($tag->closing) {
                        ...
                        break;
                    } elseif (in_array($tag->name, $this->lookFor[$startTag] ?? [], \true)) {
                        $this->pushTag($tag);
                        $res->send([$content, $tag]);
                        $this->ensureIsConsumed($tag);
                        $this->popTag();
                    } else {
                        throw new CompileException('Unexpected tag ' . substr($tag->getNotation(\true), 0, -1) . '}', $tag->position);
                    }
                }
            }
            if ($res->valid()) {
            ...
            throw new CompileException('Unexpected /} in tag ' . substr($startTag->getNotation(\true), 0, -1) . '/}', $startTag->position);
        } else {
            $this->ensureIsConsumed($startTag);
            $node = $res;
            if ($startTag->outputMode === $startTag::OutputKeepIndentation) {
    ...
    private function parseLatteTag(): Tag
    {
        ...
        $inTag = in_array($this->lexer->getState(), [TemplateLexer::StateHtmlTag, TemplateLexer::StateHtmlQuotedValue, TemplateLexer::StateHtmlComment, TemplateLexer::StateHtmlBogus], \true);
        $openToken = $stream->consume(Token::Latte_TagOpen);
        $this->lexer->pushState(TemplateLexer::StateLatteTag);
        $tag = new Tag(position: $openToken->position, closing: $closing = (bool) $stream->tryConsume(Token::Slash), name: $stream->tryConsume(Token::Latte_Name)?->text ?? ($closing ? '' : '='), tokens: $this->consumeTag(), void: (bool) $stream->tryConsume(Token::Slash), inHead: $this->inHead, inTag: $inTag, htmlElement: $this->html->getElement());
    ...
    private function getTagParser(string $name, Position $pos): callable
    {
        if (!isset($this->tagParsers[$name])) {
            $hint = ($t = Helpers::getSuggestion(array_keys($this->tagParsers), $name)) ? ", did you mean {{$t}}?" : '';
    ...
    {
        if (!$end) {
            if ($start->name !== 'syntax' && ($start->name !== 'block' || $this->tag->parent)) {
                // TODO: hardcoded
                $this->stream->throwUnexpectedException(expected: ["{/{$start->name}}"]);
            }
        } elseif ($end->name !== $start->name && $end->name !== '' || !$end->closing || $end->void) {
    ...
    public function checkBlockIsUnique(Block $block): void
    {
        ...
        if ($block->layer === Template::LayerSnippet ? isset($this->blocks[$block->layer][$name]) : isset($this->blocks[Template::LayerLocal][$name]) || isset($this->blocks[$this->blockLayer][$name])) {
    ...
    public function setContentType(string $type): static
    {
        ...
        $this->lexer->setState($type === ContentType::Html || $type === ContentType::Xml ? TemplateLexer::StateHtmlText : TemplateLexer::StatePlain);
        return $this;
    }
