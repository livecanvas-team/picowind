...
use PicowindDeps\Latte\Compiler\Nodes\Html;
use PicowindDeps\Latte\ContentType;
use PicowindDeps\Latte\Helpers;
use PicowindDeps\Latte\SecurityViolationException;
...
final class TemplateParserHtml
{
    ...
    public function inTagResolve(): ?Node
    {
        ...
        $token = $stream->peek();
        return match ($token->type) {
            Token::Html_Name => str_starts_with($token->text, TemplateLexer::NPrefix) ? $this->parseNAttribute() : $this->parseAttribute(),
    ...
    private function parseTag(): ?Node
    {
        ...
        if ($this->element && $this->parser->peekTag() === $this->element->data->tag) {
            $save = $stream->getIndex();
            $this->endName = [$endText] = $this->parseEndTag();
            if ($this->element->is($endText) || $this->element->data->textualName === $endText) {
                return null;
                // go to parseElement() one level up to close the element
            }
            $stream->seek($save);
            if (!in_array($endText, $this->element->data->unclosedTags ?? [], \true)) {
                return null;
                // go to parseElement() one level up to collapse
            }
        }
    ...
    private function parseElement(): Node
    {
        ...
        if (!$void) {
            ...
            } elseif ($outerNodes || $innerNodes || $tagNodes || $this->parser->strict || $elem->variableName || $endVariable || $elem->isRawText()) {
                $stream->throwUnexpectedException(addendum: ", expecting </{$elem->data->textualName}> for element started {$elem->position}", excerpt: $endText ? "/{$endText}>" : $stream->peek(1)?->text . $stream->peek(2)?->text);
            } else {
                // element collapsed to tags
                $res->append($content);
    ...
    private function parseStartTag(&$elem = null): Html\ElementNode
    {
        ...
        if (($this->parser->strict || $variable) && !$stream->is(Token::Whitespace, Token::Slash, Token::Html_TagClose)) {
            throw $stream->throwUnexpectedException();
        }
        $this->parser->lastIndentation = null;
        $this->parser->inHead = \false;
        $elem = new Html\ElementNode(name: $variable ? '' : $textual, position: $openToken->position, parent: $this->element, data: (object) ['tag' => $this->parser->peekTag()], contentType: $this->parser->getContentType());
    ...
    private function parseEndTag(): array
    {
        ...
        if (isset($this->element->nAttributes['syntax'])) {
            // hardcoded
            $lexer->popSyntax();
        }
    ...
    /** @return array{string, ?Nodes\Php\ExpressionNode} */
    private function parseTagName(bool $strict = \true): array
    {
        $variable = $text = null;
        $parts = [];
        $stream = $this->parser->getStream();
        ...
        $variable = $variable ? Latte\Compiler\Nodes\Php\Expression\BinaryOpNode::nest('.', ...$parts) : null;
    ...
    private function parseBogusEndTag(): ?Html\BogusTagNode
    {
        ...
        $this->parser->lastIndentation = null;
        $this->parser->inHead = \false;
        $node = new Html\BogusTagNode(openDelimiter: $openToken->text . $stream->consume(Token::Slash)->text . $stream->consume(Token::Html_Name)->text, content: new Nodes\TextNode($stream->tryConsume(Token::Whitespace)->text ?? ''), endDelimiter: $stream->consume(Token::Html_TagClose)->text, position: $openToken->position);
        $lexer->popState();
        return $node;
    }
    private function parseBogusTag(): Html\BogusTagNode
    {
        ...
        $lexer->popState();
        return new Html\BogusTagNode(openDelimiter: $openToken->text, content: $content, endDelimiter: $stream->consume(Token::Html_TagClose)->text, position: $openToken->position);
    }
    ...
    private function parseAttributeWhitespace(): Node
    {
        ...
        return $stream->is(Token::Html_Name) && str_starts_with($stream->peek()->text, TemplateLexer::NPrefix) ? new Nodes\NopNode() : new Nodes\TextNode($token->text, $token->position);
    }
    private function parseAttribute(): ?Node
    {
        ...
        if ($stream->is(Token::Latte_TagOpen)) {
            ...
            if (!$name instanceof Nodes\PrintNode) {
                return $name;
                // value like '<span {if true}attr1=val{/if}>'
            }
        } else {
            $name = $this->parser->parseText();
        }
        [$value, $quote] = $this->parseAttributeValue();
        return new Html\AttributeNode(name: $name, value: $value, quote: $quote, position: $name->position);
    }
    ...
    private function parseNAttribute(): Nodes\TextNode
    {
        ...
        $this->element->nAttributes[$name] = new Tag(name: preg_replace('~(inner-|tag-|)~', '', $name), tokens: $tokens, position: $nameToken->position, prefix: $this->getPrefix($name), inTag: \true, htmlElement: $this->element, nAttributeNode: $node = new Nodes\TextNode(''));
        return $node;
    }
    private function parseComment(): Html\CommentNode
    {
        ...
        $node = new Html\CommentNode(position: $openToken->position, content: $this->parser->parseFragment([$this->parser, 'inTextResolve']));
    ...
    private function prepareNAttrs(array $attrs, bool $void): array
    {
        ...
        if ($attrs) {
            ...
            throw new CompileException('Unexpected attribute n:' . ($hint ? "{$k}, did you mean n:{$hint}?" : implode(' and n:', array_keys($attrs))), $attrs[$k]->position);
        }
        return $res;
    }
    /**
    ...
    private function getAttrParser(string $name, Position $pos): callable
    {
        if (!isset($this->attrParsers[$name])) {
            $hint = ($t = Helpers::getSuggestion(array_keys($this->attrParsers), $name)) ? ", did you mean n:{$t}?" : '';
