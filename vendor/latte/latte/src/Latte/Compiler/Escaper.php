...
use PicowindDeps\Latte\Runtime\Filters;
use PicowindDeps\Latte\Runtime\HtmlHelpers;
...
final class Escaper
{
    public const Text = 'text', JavaScript = 'js', Css = 'css', ICal = 'ical';
    public const HtmlText = 'html', HtmlComment = 'html/comment', HtmlBogusTag = 'html/bogus', HtmlRawText = 'html/raw', HtmlTag = 'html/tag', HtmlAttribute = 'html/attr';
    private const Convertors = [self::Text => [self::HtmlText => 'escapeHtmlText', self::HtmlAttribute => 'escapeHtmlAttr', self::HtmlAttribute . '/' . self::JavaScript => 'escapeHtmlAttr', self::HtmlAttribute . '/' . self::Css => 'escapeHtmlAttr', self::HtmlComment => 'escapeHtmlComment', 'xml' => 'escapeXmlText', 'xml/attr' => 'escapeXmlAttr'], self::JavaScript => [self::HtmlText => 'escapeHtmlText', self::HtmlAttribute => 'escapeHtmlAttr', self::HtmlAttribute . '/' . self::JavaScript => 'escapeHtmlAttr', self::HtmlRawText . '/' . self::JavaScript => 'convertJSToHtmlRawText', self::HtmlComment => 'escapeHtmlComment'], self::Css => [self::HtmlText => 'escapeHtmlText', self::HtmlAttribute => 'escapeHtmlAttr', self::HtmlAttribute . '/' . self::Css => 'escapeHtmlAttr', self::HtmlRawText . '/' . self::Css => 'convertJSToHtmlRawText', self::HtmlComment => 'escapeHtmlComment'], self::HtmlText => [self::Text => 'convertHtmlToText', self::HtmlAttribute => 'convertHtmlToHtmlAttr', self::HtmlAttribute . '/' . self::JavaScript => 'convertHtmlToHtmlAttr', self::HtmlAttribute . '/' . self::Css => 'convertHtmlToHtmlAttr', self::HtmlComment => 'escapeHtmlComment', self::HtmlRawText . '/' . self::HtmlText => 'convertHtmlToHtmlRawText'], self::HtmlAttribute => [self::HtmlText => 'convertHtmlAttrToHtml']];
    private string $state = '';
    private string $tag = '';
    private string $subType = '';
    public function __construct(private string $contentType)
    {
        $this->state = in_array($contentType, [ContentType::Html, ContentType::Xml], \true) ? self::HtmlText : $contentType;
    }
    ...
    public function enterHtmlText(ElementNode $el): void
    {
        if ($el->isRawText()) {
            ...
            if ($el->is('script')) {
                ...
                $this->subType = $type === \true || $type === null ? self::JavaScript : HtmlHelpers::classifyScriptType($type);
            } elseif ($el->is('style')) {
        ...
        return match ($this->contentType) {
            ContentType::Html => match ($this->state) {
                ...
                self::HtmlRawText => match ($this->subType) {
                    self::Text => 'LR\Filters::convertJSToHtmlRawText(' . $str . ')',
                    // sanitization, escaping is not possible
            ...
            ContentType::Xml => match ($this->state) {
                self::HtmlText, self::HtmlBogusTag => 'LR\Filters::escapeXmlText(' . $str . ')',
    ...
    {
        return 'LR\Filters::convertTo($ʟ_fi, ' . var_export($this->export(), \true) . ', ' . $str . ')';
    }
