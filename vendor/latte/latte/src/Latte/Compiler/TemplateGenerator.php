...
final class TemplateGenerator
{
     ...
     */
    public function generate(Nodes\TemplateNode $node, string $className, ?string $templateName = null, bool $strictMode = \false): string
    {
        ...
        foreach ($this->constants as $name => $value) {
            $members[] = "\tpublic const {$name} = " . PhpHelpers::dump($value, \true) . ';';
        }
    ...
    private function generateBlocks(array $blocks, PrintContext $context): void
    {
        ...
        foreach ($blocks as $block) {
            if (!$block->isDynamic()) {
                $meta[$block->layer][$block->name->value] = $contentType === $block->escaping ? $block->method : [$block->method, $block->escaping];
            }
            ...
            $this->addMethod($block->method, $body, 'array $ʟ_args', 'void', $block->tag->getNotation(\true) . ' on line ' . $block->tag->position->line);
        }
        if (isset($meta)) {
    ...
    private function buildParams(string $body, array $params, string $cont, PrintContext $context): string
    {
        ...
        foreach ($params as $i => $param) {
            $res[] = $context->format('%node = %raw[%dump] ?? %raw[%dump] ?? %node;', $param->var, $cont, $i, $cont, $param->var->name, $param->default);
        }
        $extract = $params ? implode('', $res) . 'unset($ʟ_args);' : "extract({$cont});" . (str_contains($cont, '$this') ? '' : "unset({$cont});");
        return $extract . "\n\n" . $body;
    }
    /**
     ...
     * @internal
     */
    public function addMethod(string $name, string $body, string $arguments = '', string $returns = 'void', ?string $comment = null): void
    {
        $body = trim($body);
