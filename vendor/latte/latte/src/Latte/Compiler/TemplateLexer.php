...
{
    public const StatePlain = 'Plain', StateLatteTag = 'LatteTag', StateLatteComment = 'LatteComment', StateHtmlText = 'HtmlText', StateHtmlTag = 'HtmlTag', StateHtmlQuotedValue = 'HtmlQuotedValue', StateHtmlQuotedNAttrValue = 'HtmlQuotedNAttrValue', StateHtmlRawText = 'HtmlRawText', StateHtmlComment = 'HtmlComment', StateHtmlBogus = 'HtmlBogus';
    ...
    public function setSyntax(?string $type, ?string $endTag = null): static
    {
        ...
        [$this->openDelimiter, $this->closeDelimiter] = match ($type) {
            ...
            // {...}
            'off' => [$endTag ? '(?=' . $end . ')\{' : '(?!x)x', '\}'],
            'double' => $endTag ? ['(?:\{' . $left . '|(?=' . $end . ')\{)', '\}(?:\}|(?<=' . $end . '))'] : ['\{' . $left, '\}\}'],
