...
use PicowindDeps\Latte;
use PicowindDeps\Latte\CompileException;
use PicowindDeps\Latte\Compiler\Nodes\Php as Node;
...
{
    private const SchemaExpression = 'e', SchemaArguments = 'a', SchemaFilters = 'm', SchemaForeach = 'f';
    ...
    {
        return $this->parse(self::SchemaExpression, recovery: \true);
    }
    /**
    ...
    {
        return $this->parse(self::SchemaArguments, recovery: \true);
    }
    /**
    ...
    {
        return $this->isEnd() ? new Node\ModifierNode([]) : $this->parse(self::SchemaFilters);
    }
    /**
     * Parses unquoted string or PHP-like expression.
     */
    public function parseUnquotedStringOrExpression(bool $colon = \true): ExpressionNode
    {
        ...
        $end = $position->offset + strlen($parser->text) - 2;
        // 2 quotes
        do {
            $this->stream->consume();
    ...
    {
        $kind = [Token::Php_Identifier, Token::Php_Constant, Token::Php_Ellipsis, Token::Php_Array, Token::Php_Integer, Token::Php_NameFullyQualified, Token::Php_NameQualified, Token::Php_Null, Token::Php_False, Token::Php_FilterPipe, '(', ')', '<', '>', '[', ']', '|', '&', '{', '}', ':', ',', '=', '?'];
        $res = null;
    ...
    public function tryConsumeTokenBeforeUnquotedString(string ...$kind): ?Token
    {
        ...
        return $token->is(...$kind) && $this->stream->peek(1)->position->offset > $token->position->offset + strlen($token->text) ? $this->stream->consume() : null;
    }
        ...
        do {
            if (self::ActionBase[$state] === 0) {
                ...
                if ($symbol === self::SymbolNone) {
                    $recovery = $recovery ? [$this->stream->getIndex(), $state, $stateStack, $stackPos, $this->semValue, $this->semStack, $this->startTokenStack] : null;
                    $token = $token ? $this->stream->consume() : new Token(ord($schema), $schema);
                    recovery:
                    $symbol = self::TokenToSymbol[$token->type];
                }
                $idx = self::ActionBase[$state] + $symbol;
                if (($idx >= 0 && $idx < count(self::Action) && self::ActionCheck[$idx] === $symbol || $state < self::Yy2Tblstate && ($idx = self::ActionBase[$state + self::NumNonLeafStates] + $symbol) >= 0 && $idx < count(self::Action) && self::ActionCheck[$idx] === $symbol) && ($action = self::Action[$idx]) !== self::DefaultAction) {
                    /*
                    ...
                    */
                    if ($action > 0) {
                        ...
                        $rule = $action - self::NumNonLeafStates;
                        // shift-and-reduce
                    } else {
                        $rule = -$action;
                    }
            ...
            do {
                ...
                } elseif ($rule !== self::UnexpectedTokenRule) {
                    // reduce
                ...
                } elseif ($recovery && $this->isExpectedEof($state)) {
                    // recoverable error
                ...
                $rule = $state - self::NumNonLeafStates;
                // shift-and-reduce
            } while (\true);
        } while (\true);
    }
    ...
    private function isExpectedEof(int $state): bool
    {
        foreach (self::SymbolToName as $symbol => $name) {
            $idx = self::ActionBase[$state] + $symbol;
            if (($idx >= 0 && $idx < count(self::Action) && self::ActionCheck[$idx] === $symbol || $state < self::Yy2Tblstate && ($idx = self::ActionBase[$state + self::NumNonLeafStates] + $symbol) >= 0 && $idx < count(self::Action) && self::ActionCheck[$idx] === $symbol) && self::Action[$idx] !== self::UnexpectedTokenRule && self::Action[$idx] !== self::DefaultAction && $symbol === 0) {
                return \true;
            }
        }
        return \false;
    ...
    protected function checkFunctionName(Expression\FunctionCallNode|Expression\FunctionCallableNode $func): ExpressionNode
    {
    ...
    protected static function handleBuiltinTypes(NameNode $name): NameNode|Node\IdentifierNode
    {
        ...
        return isset($builtinTypes[$lowerName]) ? new Node\IdentifierNode($lowerName, $name->position) : $name;
    }
    ...
    protected function parseDocString(string $startToken, array $parts, string $endToken, Position $startPos, Position $endPos): Scalar\StringNode|Scalar\InterpolatedStringNode
    {
        ...
        if (str_contains($indentation, ' ') && str_contains($indentation, "\t")) {
            ...
            $this->stripIndentation('', $indentation, \true, \false, $parts[0]->position);
        }
        $newParts = [];
        foreach ($parts as $i => $part) {
            if ($part instanceof Node\InterpolatedStringPartNode) {
                $isLast = $i === count($parts) - 1;
                $part->value = $this->stripIndentation($part->value, $indentation, $i === 0, $isLast, $part->position);
                if ($isLast) {
    ...
    private function stripIndentation(string $str, string $indentation, bool $atStart, bool $atEnd, Position $position): string
    {
        if ($indentation === '') {
            return $str;
        }
        ...
        $end = $atEnd ? '(?:(?=[\r\n])|\z)' : '(?=[\r\n])';
        $regex = '/' . $start . '([ \t]*)(' . $end . ')?/D';
        return preg_replace_callback($regex, function ($matches) use ($indentation, $position) {
            ...
            if (str_contains($prefix, $indentation[0] === ' ' ? "\t" : ' ')) {
                ...
                throw new CompileException('Invalid body indentation level ' . '(expecting an indentation level of at least ' . $indentLen . ')', $position);
            }
    ...
    public function convertArrayToList(Expression\ArrayNode $array): Node\ListNode
    {
        ...
        foreach ($array->items as $item) {
            ...
            $items[] = $value ? new Node\ListItemNode($value, $item->key, $item->byRef, $item->position) : null;
        }
