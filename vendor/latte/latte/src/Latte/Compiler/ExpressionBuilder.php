...
use PicowindDeps\Latte\Compiler\Nodes\Php\ArgumentNode;
...
{
    public function __construct(private ExpressionNode|NameNode $expr)
    {
    }
    ...
    {
        $name = is_string($name) ? new NameNode($name) : ($name instanceof self ? $name->expr : $name);
    ...
    {
        $name = is_string($name) ? new IdentifierNode($name) : ($name instanceof self ? $name->expr : $name);
    ...
    private static function arrayToArgs(array $arr): array
    {
        ...
        foreach ($arr as $key => $arg) {
            $args[] = $arg instanceof ArgumentNode ? $arg : new ArgumentNode(self::valueToNode($arg), name: is_string($key) ? new IdentifierNode($key) : null);
        }
        return $args;
    }
}
