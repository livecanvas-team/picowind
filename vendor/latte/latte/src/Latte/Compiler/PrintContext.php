...
use PicowindDeps\Latte\Compiler\Nodes\Php as Nodes;
...
{
    /** associativity */
    private const Left = -1, None = 0, Right = 1;
    ...
    {
        $pos = 0;
        // enumerate arguments except for %escape
        $mask = preg_replace_callback('#%([a-z]{3,})#i', function ($m) use (&$pos) {
            return $m[1] === 'escape' ? '%0.escape' : '%' . $pos++ . '.' . $m[1];
        }, $mask);
        $mask = preg_replace_callback('#% (\d+) \. (escape|modify(?:Content)?) ( \( ([^()]*+|(?-2))+ \) )#xi', function ($m) use ($args) {
            [, $pos, $fn, $var] = $m;
            $var = substr($var, 1, -1);
        ...
        return preg_replace_callback('#([,+]?\s*)? % (\d+) \. ([a-z]{3,}) (\?)? (\s*\+\s*)? ()#xi', function ($m) use ($args) {
            ...
            return $code === '' ? trim($left) . $right : $left . $code . $right;
        }, $mask);
    }
    ...
    {
        return $quote === "'" ? "'" . addcslashes($str, "'\\") . "'" : '"' . addcslashes($str, "\n\r\t\f\v\$\"\\") . '"';
    }
    /**
    ...
    public function infixOp(Node $node, Node $leftNode, string $operatorString, Node $rightNode): string
    {
        ...
        return $this->parenthesize($leftNode, $precedence, self::Left) . $operatorString . $this->parenthesize($rightNode, $precedence, self::Right);
    }
    /**
    ...
    {
        return $this->operatorPrecedence[match (\true) {
    ...
    {
        return $node instanceof Nodes\NameNode || $node instanceof Nodes\IdentifierNode ? (string) $node : '{' . $node->print($this) . '}';
    }
    public function memberAsString(Node $node): string
    {
        return $node instanceof Nodes\NameNode || $node instanceof Nodes\IdentifierNode ? $this->encodeString((string) $node) : $node->print($this);
    }
    /**
    ...
    {
        return $expr instanceof Nodes\NameNode || $expr instanceof Expression\VariableNode || $expr instanceof Expression\ArrayAccessNode || $expr instanceof Expression\FunctionCallNode || $expr instanceof Expression\FunctionCallableNode || $expr instanceof Expression\MethodCallNode || $expr instanceof Expression\MethodCallableNode || $expr instanceof Expression\StaticMethodCallNode || $expr instanceof Expression\StaticMethodCallableNode || $expr instanceof Expression\ArrayNode ? $expr->print($this) : '(' . $expr->print($this) . ')';
    }
    /**
    ...
    {
        return $expr instanceof Expression\VariableNode || $expr instanceof Nodes\NameNode || $expr instanceof Expression\ArrayAccessNode || $expr instanceof Expression\PropertyFetchNode || $expr instanceof Expression\StaticPropertyFetchNode || $expr instanceof Expression\FunctionCallNode || $expr instanceof Expression\FunctionCallableNode || $expr instanceof Expression\MethodCallNode || $expr instanceof Expression\MethodCallableNode || $expr instanceof Expression\StaticMethodCallNode || $expr instanceof Expression\StaticMethodCallableNode || $expr instanceof Expression\ArrayNode || $expr instanceof Scalar\StringNode || $expr instanceof Scalar\BooleanNode || $expr instanceof Scalar\NullNode || $expr instanceof Expression\ConstantFetchNode || $expr instanceof Expression\ClassConstantFetchNode ? $expr->print($this) : '(' . $expr->print($this) . ')';
    }
    /**
    ...
    {
        return $name instanceof Scalar\StringNode ? $name->print($this) : $this->format('(LR\Helpers::stringOrNull($ʟ_tmp = %node) ?? throw new InvalidArgumentException(sprintf(%dump, get_debug_type($ʟ_tmp))))', $name, $entity . ' must be a string, %s given.');
    }
}
