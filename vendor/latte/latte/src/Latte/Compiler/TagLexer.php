...
final class TagLexer
{
    ...
    {
        preg_match($colon ? '~ ( [./@_a-z0-9#!-] | :(?!:) | \{\$ [_a-z0-9\[\]()>-]+ } )++  (?=\s+[!"\'$(\[{,\\\\|\~\w-] | [,|]  | \s*$) ~xAi' : '~ ( [./@_a-z0-9#!-]          | \{\$ [_a-z0-9\[\]()>-]+ } )++  (?=\s+[!"\'$(\[{,\\\\|\~\w-] | [,:|] | \s*$) ~xAi', $input, $match, offset: $position->offset - $offsetDelta);
        $position = new Position($position->line, $position->column - 1, $position->offset - 1);
        return $match && !is_numeric($match[0]) ? $this->tokenize('"' . $match[0] . '"', $position) : null;
    }
...
XX;
        ...
        foreach ($matches as $m) {
            if (isset($m['char'])) {
                ...
                goto matchRE;
            } elseif (isset($m[$type = 'Php_ObjectOperator']) || isset($m[$type = 'Php_NullsafeObjectOperator']) || isset($m[$type = 'Php_UndefinedsafeObjectOperator'])) {
                ...
                $this->addToken(self::Keywords[$lower] ?? (preg_match('~[A-Z_][A-Z0-9_]{2,}$~DA', $m['Php_Identifier']) ? Token::Php_Constant : Token::Php_Identifier), $m['Php_Identifier']);
                ...
                isset($m['rest']) ? $this->addToken(Token::Php_ConstantEncapsedString, "'" . $m['rest']) : throw new CompileException('Unterminated string.', $this->position);
                ...
                $this->addToken(null, '"');
                if (count($this->tokens) <= $count + 2 && (!$token || $token->type === Token::Php_EncapsedAndWhitespace)) {
                ...
                if (str_contains($m['Php_StartHeredoc'], "'")) {
                    // nowdoc
                ...
                isset($m['rest']) ? $this->addToken(Token::Php_Comment, '/*' . $m['rest']) : throw new CompileException('Unterminated comment.', $this->position);
 ...
 . "\n\t\t\t((?<end>  {$endRe}  )  .*  )|\n\t\t\t(?<char>  ( \\\\. | [^\\\\] )  )\n\t\t\t~xsA";
        ...
        foreach ($matches as $m) {
            ...
            if (isset($m['Php_CurlyOpen'])) {
                ...
                if (isset($m[$type = 'Php_ObjectOperator']) || isset($m[$type = 'Php_NullsafeObjectOperator']) || isset($m[$type = 'Php_UndefinedsafeObjectOperator'])) {
                    $this->addToken(constant(Token::class . '::' . $type), $m[$type]);
                    $this->addToken(Token::Php_Identifier, $m['Php_Identifier']);
