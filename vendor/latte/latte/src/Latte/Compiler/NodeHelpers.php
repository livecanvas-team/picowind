...
final class NodeHelpers
{
    ...
    {
        $found = [];
        (new NodeTraverser())->traverse($node, enter: function (Node $node) use ($filter, &$found) {
            if ($filter($node)) {
                $found[] = $node;
            }
        });
    ...
    {
        return (new NodeTraverser())->traverse($node, enter: fn(Node $node) => clone $node);
    }
    public static function toValue(ExpressionNode $node, bool $constants = \false): mixed
    {
        if ($node instanceof Scalar\BooleanNode || $node instanceof Scalar\FloatNode || $node instanceof Scalar\IntegerNode || $node instanceof Scalar\StringNode) {
            return $node->value;
            ...
            foreach ($node->items as $item) {
                ...
                if ($item->key) {
                    $key = $item->key instanceof Php\IdentifierNode ? $item->key->name : self::toValue($item->key, $constants);
                    $res[$key] = $value;
                } elseif ($item->unpack) {
            ...
            return defined($name) ? constant($name) : throw new \InvalidArgumentException("The constant '{$name}' is not defined.");
        } elseif ($node instanceof Expression\ClassConstantFetchNode && $constants) {
            $class = $node->class instanceof Php\NameNode ? $node->class->toCodeString() : self::toValue($node->class, $constants);
