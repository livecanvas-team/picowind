...
final class PhpHelpers
{
    ...
    public static function optimizeEcho(string $source): string
    {
        ...
        for ($i = 0; $i < count($tokens); $i++) {
            ...
            if ($token[0] === T_ECHO) {
                ...
                if ($str !== '') {
                    $res = substr_replace($res, 'echo ' . ($str === "\n" ? '"\n"' : var_export($str, \true)), $start, strlen($res) - $start);
                }
    ...
    public static function decodeEscapeSequences(string $str, ?string $quote): string
    {
        ...
        return preg_replace_callback('~\\\\([\\\\$nrtfve]|[xX][0-9a-fA-F]{1,2}|[0-7]{1,3}|u\{([0-9a-fA-F]+)\})~', function ($matches) {
            $ch = $matches[1];
            $replacements = ['\\' => '\\', '$' => '$', 'n' => "\n", 'r' => "\r", 't' => "\t", 'f' => "\f", 'v' => "\v", 'e' => "\x1b"];
    ...
    private static function codePointToUtf8(int $num): string
    {
        return match (\true) {
            ...
            $num <= 0x1fffff => chr(($num >> 18) + 0xf0) . chr(($num >> 12 & 0x3f) + 0x80) . chr(($num >> 6 & 0x3f) + 0x80) . chr(($num & 0x3f) + 0x80),
    ...
    {
        $process = proc_open($phpBinary . ' -l -n', [['pipe', 'r'], ['pipe', 'w'], ['pipe', 'w']], $pipes, null, null, ['bypass_shell' => \true]);
        ...
        $position = preg_match('~ on line (\d+)~', $error, $m) ? new Position((int) $m[1], 0) : null;
        $error = preg_replace('~(^Fatal error: | in Standard input code| on line \d+)~', '', $error);
        throw (new CompileException('Error in generated code: ' . trim($error), $position))->setSource($code, $name);
    }
}
