...
{
    public function __construct(public int $line = 1, public int $column = 1, public int $offset = 0)
    {
    }
    public function advance(string $str): self
    {
        if ($lines = substr_count($str, "\n")) {
            return new self($this->line + $lines, strlen($str) - strrpos($str, "\n"), $this->offset + strlen($str));
        } else {
            return new self($this->line, $this->column + strlen($str), $this->offset + strlen($str));
        }
    }
