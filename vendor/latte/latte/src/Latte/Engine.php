...
namespace PicowindDeps\Latte;

use PicowindDeps\Latte\Compiler\Nodes\TemplateNode;
...
class Engine
{
    ...
    public const VERSION = self::Version, VERSION_ID = self::VersionId;
    /** @deprecated use ContentType::* */
    public const CONTENT_HTML = ContentType::Html, CONTENT_XML = ContentType::Xml, CONTENT_JS = ContentType::JavaScript, CONTENT_CSS = ContentType::Css, CONTENT_ICAL = ContentType::ICal, CONTENT_TEXT = ContentType::Text;
     ...
     * @param  mixed[]  $params
     */
    public function createTemplate(string $name, array $params = [], bool $clearCache = \true): Runtime\Template
    {
    ...
    public function parse(string $template): TemplateNode
    {
        ...
        return $parser->setContentType($this->contentType)->setPolicy($this->getPolicy(effective: \true))->parse($template);
    }
    /**
     * Calls node visitors.
     */
    ...
    public function generate(TemplateNode $node, string $name): string
    {
        ...
        return $generator->generate($node, $this->getTemplateClass($name), $name, $this->strictTypes);
    }
    /**
    ...
    private function loadTemplate(string $name): string
    {
        ...
        if (class_exists($class, \false)) {
            ...
            if (@eval(substr($compiled, 5)) === \false) {
                ...
                throw (new CompileException('Error in template: ' . error_get_last()['message']))->setSource($compiled, "{$name} (compiled)");
            }
        }
        return $class;
    }
    ...
    {
        return [$this->contentType, $this->strictTypes, $this->strictParsing, $this->syntax, array_map(fn($extension) => [get_debug_type($extension), $extension->getCacheKey($this)], $this->extensions)];
    }
    /**
    ...
    {
        return !$effective || $this->sandboxed ? $this->policy : null;
    }
    ...
    public function setSandboxMode(bool $state = \true): static
    {
     ...
     */
    public function setAutoRefresh(bool $state = \true): static
    {
     ...
     */
    public function setStrictTypes(bool $state = \true): static
    {
        $this->strictTypes = $state;
        return $this;
    }
    public function setStrictParsing(bool $state = \true): static
    {
    ...
    public function __get(string $name)
    {
        if ($name === 'onCompile') {
            ...
            $loc = isset($trace['file'], $trace['line']) ? ' (in ' . $trace['file'] . ' on ' . $trace['line'] . ')' : '';
