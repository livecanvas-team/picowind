...
class FilterExecutor
{
    ...
    public function __get(string $name): callable
    {
        ...
        return $this->{$name} = $infoAware ? fn(...$args) => $this->callInfoAwareAsClassic($callback, ...$args) : $callback;
    }
    /**
    ...
    public function filterContent(string $name, FilterInfo $info, mixed ...$args): mixed
    {
        ...
        if ($info->contentType !== ContentType::Text) {
            throw new Latte\RuntimeException("Filter |{$name} is called with incompatible content type " . strtoupper($info->contentType ?? 'NULL') . ($info->contentType === ContentType::Html ? ', try to prepend |stripHtml.' : '.'));
        }
        $res = $callback(...$args);
    ...
    private function callInfoAwareAsClassic(callable $filter, mixed ...$args): mixed
    {
        ...
        $res = $filter(...$args);
        return $info->contentType === ContentType::Html ? new Html($res) : $res;
    }
}
