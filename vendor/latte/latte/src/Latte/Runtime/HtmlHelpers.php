...
final class HtmlHelpers
{
    ...
    {
        static $names = ['img' => 1, 'hr' => 1, 'br' => 1, 'input' => 1, 'meta' => 1, 'area' => 1, 'embed' => 1, 'keygen' => 1, 'source' => 1, 'base' => 1, 'col' => 1, 'link' => 1, 'param' => 1, 'basefont' => 1, 'frame' => 1, 'isindex' => 1, 'wbr' => 1, 'command' => 1, 'track' => 1];
    ...
    public static function isUrlAttribute(string $tag, string $attr): bool
    {
        ...
        return in_array($attr, ['href', 'src', 'action', 'formaction'], \true) || $attr === 'data' && strtolower($tag) === 'object';
    }
    /**
    ...
    public static function validateTagChange(mixed $name, ?string $origName = null): string
    {
        ...
        } elseif (self::isVoidElement($name) !== self::isVoidElement($origName ?? 'div') || in_array(strtolower($name), ['style', 'script'], \true)) {
