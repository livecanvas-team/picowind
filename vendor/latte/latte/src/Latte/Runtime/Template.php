...
namespace PicowindDeps\Latte\Runtime;

use PicowindDeps\Latte;
use PicowindDeps\Latte\Compiler\Escaper;
use PicowindDeps\Latte\Engine;
...
class Template
{
    public const LayerTop = 0, LayerSnippet = 'snippet', LayerLocal = 'local';
     ...
     * @param  mixed[]  $params
     */
    public function __construct(Engine $engine, array $params, FilterExecutor $filters, \stdClass $providers, string $name)
    {
        $this->engine = $engine;
        $this->params = $params;
     ...
     * @internal
     */
    public function renderBlock(string $name, array $params, string|\Closure|null $mod = null, int|string|null $layer = null): void
    {
        $block = $layer ? $this->blocks[$layer][$name] ?? null : $this->blocks[self::LayerLocal][$name] ?? $this->blocks[self::LayerTop][$name] ?? null;
        if (!$block) {
            $hint = $layer && ($t = Latte\Helpers::getSuggestion($this->getBlockNames($layer), $name)) ? ", did you mean '{$t}'?" : '.';
            $name = $layer ? "{$layer} {$name}" : $name;
            throw new Latte\RuntimeException("Cannot include undefined block '{$name}'{$hint}");
        }
        $this->filter(fn() => reset($block->functions)($params), $mod, $block->contentType, "block {$name}");
    }
    /**
    ...
    {
        $this->filter(fn() => $this->render($block), $mod, static::ContentType, "'{$this->name}'");
    }
    /**
     * Renders template.
     * @param  mixed[]  $params
    ...
    public function createTemplate(string $name, array $params, string $relation): self
    {
        ...
        $child = $relation === 'sandbox' ? (clone $this->engine)->setSandboxMode()->createTemplate($name, $params) : $this->engine->createTemplate($name, $params, clearCache: \false);
        $child->referringTemplate = $this;
        $child->referenceType = $relation;
        $child->global = $this->global;
        if (in_array($relation, ['extends', 'includeblock', 'import', 'embed'], \true)) {
    ...
    private function filter(callable $producer, string|\Closure|null $targetType, string $contentType, string $name): void
    {
        if ($targetType === null || $targetType === $contentType) {
            ...
            throw new Latte\RuntimeException(sprintf("Including {$name} with content type %s into incompatible type %s.", strtoupper($contentType), strtoupper($targetType)));
        }
    }
    /**
     ...
     * @internal
     */
    protected function addBlock(string $name, string $contentType, array $functions, int|string|null $layer = null): void
    {
