...
namespace PicowindDeps\Latte\Sandbox;
...
final class SandboxExtension extends Latte\Extension
{
    ...
    {
        $this->policy = $engine->getPolicy(effective: \true);
    }
    public function getTags(): array
    {
        return ['sandbox' => [Nodes\SandboxNode::class, 'create']];
    }
    public function getPasses(): array
    {
        return $this->policy ? ['sandbox' => self::order([$this, 'processPass'], before: '*')] : [];
    }
    ...
    {
        return (bool) $engine->getPolicy(effective: \true);
    }
    ...
    private function sandboxVisitor(Node $node): Node
    {
        if ($node instanceof Expression\VariableNode) {
            ...
            if (!$this->policy->isFunctionAllowed((string) $node->name)) {
                ...
                $arg = new Expression\AuxiliaryNode(fn(PrintContext $context, ...$args) => '$this->global->sandbox->args(' . $context->implode($args) . ')', $node->args);
                $node->args = [new Php\ArgumentNode($arg, unpack: \true)];
            }
            return $node;
            ...
            return $node;
        } elseif ($node instanceof Expression\PropertyFetchNode || $node instanceof Expression\StaticPropertyFetchNode || $node instanceof Expression\FunctionCallNode || $node instanceof Expression\FunctionCallableNode || $node instanceof Expression\MethodCallNode || $node instanceof Expression\MethodCallableNode || $node instanceof Expression\StaticMethodCallNode || $node instanceof Expression\StaticMethodCallableNode) {
