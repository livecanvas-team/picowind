...
{
    public function __construct(public Latte\Policy $policy)
    {
    }
    ...
    {
        foreach ($args as $arg) {
            if (is_array($arg) && is_callable($arg, \true, $text) && !$this->policy->isMethodAllowed(is_object($arg[0]) ? $arg[0]::class : $arg[0], $arg[1])) {
    ...
    private function checkCallable(mixed $callable): void
    {
        if (!is_callable($callable)) {
            ...
            $allowed = $callable instanceof \Closure ? \true : $this->policy->isMethodAllowed($callable::class, '__invoke');
        } else {
            $allowed = \false;
        }
        if (!$allowed) {
