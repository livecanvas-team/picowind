...
final class Cache
{
    ...
    public function loadOrCreate(Engine $engine, string $name): void
    {
        ...
        $signature = $this->autoRefresh ? md5(serialize($this->generateRefreshSignature($engine, $name))) : null;
        $lock = defined('PHP_WINDOWS_VERSION_BUILD') || $signature ? $this->acquireLock("{$file}.lock", LOCK_SH) : null;
        if (!($signature && $signature !== stream_get_contents($lock)) && @(include $file) !== \false) {
            return;
        }
        if ($lock) {
            flock($lock, LOCK_UN);
        ...
        if (!is_file($file) || $signature && $signature !== stream_get_contents($lock)) {
            ...
            if (file_put_contents("{$file}.tmp", $compiled) !== strlen($compiled) || !rename("{$file}.tmp", $file)) {
                @unlink("{$file}.tmp");
                // @ - file may not exist
    ...
    {
        return [Engine::Version, $engine->getLoader()->getContent($name), array_map(fn($extension) => filemtime((new \ReflectionObject($extension))->getFileName()), $engine->getExtensions())];
    }
}
