...
use PicowindDeps\Predis\Connection\Aggregate\PredisCluster;
...
use Relay\Relay;
use PicowindDeps\Symfony\Component\Cache\CacheItem;
use PicowindDeps\Symfony\Component\Cache\Exception\InvalidArgumentException;
use PicowindDeps\Symfony\Component\Cache\Exception\LogicException;
use PicowindDeps\Symfony\Component\Cache\Marshaller\DeflateMarshaller;
use PicowindDeps\Symfony\Component\Cache\Marshaller\MarshallerInterface;
use PicowindDeps\Symfony\Component\Cache\Marshaller\TagAwareMarshaller;
...
class RedisTagAwareAdapter extends AbstractTagAwareAdapter
{
    ...
    public function __construct(\Redis|Relay|\RedisArray|\RedisCluster|\PicowindDeps\Predis\ClientInterface $redis, string $namespace = '', int $defaultLifetime = 0, ?MarshallerInterface $marshaller = null)
    {
        if ($redis instanceof \PicowindDeps\Predis\ClientInterface && $redis->getConnection() instanceof ClusterInterface && !$redis->getConnection() instanceof PredisCluster) {
            throw new InvalidArgumentException(\sprintf('Unsupported Predis cluster connection: only "%s" is, "%s" given.', PredisCluster::class, get_debug_type($redis->getConnection())));
        }
        $isRelay = $redis instanceof Relay;
        if ($isRelay || \defined('PicowindDeps\Redis::OPT_COMPRESSION') && \in_array($redis::class, [\Redis::class, \RedisArray::class, \RedisCluster::class], \true)) {
...
EOLUA;
        $results = $this->pipeline(function () use ($ids, $lua) {
            foreach ($ids as $id) {
                yield 'eval' => $this->redis instanceof \PicowindDeps\Predis\ClientInterface ? [$lua, 1, $id] : [$lua, [$id], 1];
            }
        });
        foreach ($results as $id => $result) {
...
EOLUA;
        $results = $this->pipeline(function () use ($tagIds, $lua) {
            ...
            foreach ($tagIds as $id) {
                yield 'eval' => $this->redis instanceof \PicowindDeps\Predis\ClientInterface ? [$lua, 1, $id, $prefix] : [$lua, [$id, $prefix], 1];
            }
        });
        $lua = <<<'EOLUA'
            redis.replicate_commands()
