 ...
 */
namespace PicowindDeps\Symfony\Component\Cache\Traits;

use PicowindDeps\Predis\Command\Redis\UNLINK;
use PicowindDeps\Predis\Connection\Aggregate\ClusterInterface;
use PicowindDeps\Predis\Connection\Aggregate\RedisCluster;
use PicowindDeps\Predis\Connection\Aggregate\ReplicationInterface;
use PicowindDeps\Predis\Connection\Cluster\ClusterInterface as Predis2ClusterInterface;
use PicowindDeps\Predis\Connection\Cluster\RedisCluster as Predis2RedisCluster;
use PicowindDeps\Predis\Connection\Replication\ReplicationInterface as Predis2ReplicationInterface;
use PicowindDeps\Predis\Response\ErrorInterface;
use PicowindDeps\Predis\Response\Status;
use Relay\Relay;
use Relay\Sentinel;
use PicowindDeps\Symfony\Component\Cache\Exception\CacheException;
use PicowindDeps\Symfony\Component\Cache\Exception\InvalidArgumentException;
use PicowindDeps\Symfony\Component\Cache\Marshaller\DefaultMarshaller;
...
trait RedisTrait
{
    ...
    private \Redis|Relay|\RedisArray|\RedisCluster|\PicowindDeps\Predis\ClientInterface $redis;
    private MarshallerInterface $marshaller;
    private function init(\Redis|Relay|\RedisArray|\RedisCluster|\PicowindDeps\Predis\ClientInterface $redis, string $namespace, int $defaultLifetime, ?MarshallerInterface $marshaller): void
    {
        parent::__construct($namespace, $defaultLifetime);
        if (preg_match('#[^-+_.A-Za-z0-9]#', $namespace, $match)) {
            throw new InvalidArgumentException(\sprintf('RedisAdapter namespace contains "%s" but only characters in [-+_.A-Za-z0-9] are allowed.', $match[0]));
        }
        if ($redis instanceof \PicowindDeps\Predis\ClientInterface && $redis->getOptions()->exceptions) {
    ...
    public static function createConnection(#[\SensitiveParameter] string $dsn, array $options = []): \Redis|\RedisArray|\RedisCluster|\PicowindDeps\Predis\ClientInterface|Relay
    {
        ...
        if (!\extension_loaded('redis') && !\extension_loaded('relay') && !class_exists(\PicowindDeps\Predis\Client::class)) {
        ...
        $params['auth'] ??= $auth;
        if (isset($params['redis_sentinel']) && !class_exists(\PicowindDeps\Predis\Client::class) && !class_exists(\RedisSentinel::class) && !class_exists(Sentinel::class)) {
        ...
        $class = $params['class'] ?? match (\true) {
            $params['redis_cluster'] => \extension_loaded('redis') ? \RedisCluster::class : \PicowindDeps\Predis\Client::class,
            isset($params['redis_sentinel']) => match (\true) {
                \extension_loaded('redis') => \Redis::class,
                \extension_loaded('relay') => Relay::class,
                default => \PicowindDeps\Predis\Client::class,
            },
        ...
        if (isset($params['redis_sentinel']) && !is_a($class, \PicowindDeps\Predis\Client::class, \true) && !class_exists(\RedisSentinel::class) && !class_exists(Sentinel::class)) {
        ...
        if ($isRedisExt || $isRelayExt) {
            ...
            $initializer = static function () use ($class, $isRedisExt, $connect, $params, $auth, $hosts, $tls) {
                ...
                do {
                    ...
                    $passAuth = null !== $params['auth'] && (!$isRedisExt || \defined('PicowindDeps\Redis::OPT_NULL_MULTIBULK_AS_NULL'));
                    $address = \false;
                ...
                try {
                    ...
                    @$redis->{$connect}($host, $port, (float) $params['timeout'], (string) $params['persistent_id'], $params['retry_interval'], $params['read_timeout'], ...\defined('PicowindDeps\Redis::SCAN_PREFIX') || !$isRedisExt ? [$extra] : []);
                    ...
                    if (0 < $params['tcp_keepalive'] && (!$isRedisExt || \defined('PicowindDeps\Redis::OPT_TCP_KEEPALIVE'))) {
                        $redis->setOption($isRedisExt ? \Redis::OPT_TCP_KEEPALIVE : Relay::OPT_TCP_KEEPALIVE, $params['tcp_keepalive']);
                    }
                    if (!\defined('PicowindDeps\Redis::SCAN_PREFIX') && null !== $auth && $isRedisExt && !$redis->auth($auth) || !$redis->select($params['dbindex'])) {
            ...
            $initializer = static function () use ($isRedisExt, $class, $params, $hosts) {
                ...
                try {
                    $redis = new $class(null, $hosts, $params['timeout'], $params['read_timeout'], (bool) $params['persistent'], $params['auth'] ?? '', ...\defined('PicowindDeps\Redis::SCAN_PREFIX') ? [$params['ssl'] ?? null] : []);
                } catch (\RedisClusterException $e) {
                    throw new InvalidArgumentException('Redis connection failed: ' . $e->getMessage());
                }
                if (0 < $params['tcp_keepalive'] && (!$isRedisExt || \defined('PicowindDeps\Redis::OPT_TCP_KEEPALIVE'))) {
        ...
        } elseif (is_a($class, \PicowindDeps\Predis\ClientInterface::class, \true)) {
    ...
    protected function doFetch(array $ids): iterable
    {
        ...
        $result = [];
        if ($this->redis instanceof \PicowindDeps\Predis\ClientInterface && ($this->redis->getConnection() instanceof ClusterInterface || $this->redis->getConnection() instanceof Predis2ClusterInterface) || $this->redis instanceof RelayCluster) {
    ...
    {
        if ($this->redis instanceof \PicowindDeps\Predis\ClientInterface) {
        ...
        $hosts = $this->getHosts();
        $host = reset($hosts);
        if ($host instanceof \PicowindDeps\Predis\Client) {
        ...
        foreach ($hosts as $host) {
            ...
            } elseif (!$host instanceof \PicowindDeps\Predis\ClientInterface) {
                $prefix = \defined('PicowindDeps\Redis::SCAN_PREFIX') && \Redis::SCAN_PREFIX & $host->getOption(\Redis::OPT_SCAN) ? '' : $host->getOption(\Redis::OPT_PREFIX);
            ...
            if (!version_compare($info['redis_version'], '2.8', '>=')) {
                ...
                $args = $this->redis instanceof \PicowindDeps\Predis\ClientInterface ? [0, $pattern] : [[$pattern], 0];
            ...
            $cursor = null;
            do {
                $keys = $host instanceof \PicowindDeps\Predis\ClientInterface ? $host->scan($cursor ?? 0, 'MATCH', $pattern, 'COUNT', 1000) : $host->scan($cursor, $pattern, 1000);
    ...
    protected function doDelete(array $ids): bool
    {
        ...
        if ($this->redis instanceof \PicowindDeps\Predis\ClientInterface && ($this->redis->getConnection() instanceof ClusterInterface || $this->redis->getConnection() instanceof Predis2ClusterInterface)) {
            static $del;
    ...
    private function pipeline(\Closure $generator, ?object $redis = null): \Generator
    {
        $ids = [];
        $redis ??= $this->redis;
        if ($redis instanceof \RedisCluster || $redis instanceof \PicowindDeps\Predis\ClientInterface && ($redis->getConnection() instanceof RedisCluster || $redis->getConnection() instanceof Predis2RedisCluster)) {
            ...
            foreach ($generator() as $command => $args) {
                ...
                $ids[] = 'eval' === $command ? $redis instanceof \PicowindDeps\Predis\ClientInterface ? $args[2] : $args[1][0] : $args[0];
            }
        } elseif ($redis instanceof \PicowindDeps\Predis\ClientInterface) {
        ...
        if (!$redis instanceof \PicowindDeps\Predis\ClientInterface && 'eval' === $command && $redis->getLastError()) {
        ...
        if ($this->redis instanceof \PicowindDeps\Predis\ClientInterface) {
            ...
            if (($connection instanceof ClusterInterface || $connection instanceof Predis2ClusterInterface) && $connection instanceof \Traversable) {
                ...
                foreach ($connection as $c) {
                    $hosts[] = new \PicowindDeps\Predis\Client($c);
                }
            }
