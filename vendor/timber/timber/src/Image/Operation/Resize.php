...
namespace PicowindDeps\Timber\Image\Operation;

use Imagick;
use PicowindDeps\Timber\Helper;
use PicowindDeps\Timber\Image\Operation as ImageOperation;
use PicowindDeps\Timber\ImageHelper;
use WP_Image_Editor;
/**
...
class Resize extends ImageOperation
{
    ...
    public function filename($src_filename, $src_extension)
    {
        ...
        $result = $src_filename . '-' . $w . 'x' . $h . '-c-' . ($this->crop ?: 'f');
    ...
    protected function run_animated_gif($load_filename, $save_filename, WP_Image_Editor $editor)
    {
        $w = $this->w;
        $h = $this->h;
        if (!\class_exists('Imagick') || \defined('PicowindDeps\TEST_NO_IMAGICK') && TEST_NO_IMAGICK) {
    ...
    public function run($load_filename, $save_filename)
    {
        ...
        if (!\is_wp_error($image)) {
            ...
            $image->crop((int) \round($crop['x']), (int) \round($crop['y']), (int) \round($crop['src_w']), (int) \round($crop['src_h']), (int) \round($crop['target_w']), (int) \round($crop['target_h']));
