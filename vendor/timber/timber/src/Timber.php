...
use PicowindDeps\Timber\Factory\CommentFactory;
use PicowindDeps\Timber\Factory\MenuFactory;
use PicowindDeps\Timber\Factory\PagesMenuFactory;
use PicowindDeps\Timber\Factory\PostFactory;
use PicowindDeps\Timber\Factory\TermFactory;
use PicowindDeps\Timber\Factory\UserFactory;
use PicowindDeps\Timber\Integration\IntegrationInterface;
use WP_Comment;
...
class Timber
{
    ...
    {
        \defined("PicowindDeps\\TIMBER_LOC") or \define("PicowindDeps\\TIMBER_LOC", \realpath(\dirname(__DIR__)));
    }
    /**
     * @codeCoverageIgnore
     */
    ...
    {
        if (!\defined('ABSPATH') || !\class_exists('\WP') || \defined('PicowindDeps\TIMBER_LOADED')) {
            return;
        }
        $self = new self();
        $self->init_constants();
         ...
         */
        \class_alias(Timber::class, 'PicowindDeps\Timber');
        \define('PicowindDeps\TIMBER_LOADED', \true);
    }
    /**
    ...
    private static function check_post_api_deprecations($query = \false, $options = [], string $function_name = 'Timber::get_post()')
    {
        if (\is_string($query) && !\is_numeric($query)) {
            Helper::doing_it_wrong($function_name, 'Getting a post by post slug or post name was removed from Timber::get_post() in Timber 2.0. Use Timber::get_post_by() instead.', '2.0.0');
        }
        if (\is_string($options)) {
            Helper::doing_it_wrong($function_name, 'The $PostClass parameter for passing in the post class to use in Timber::get_posts() was replaced with an $options array in Timber 2.0. To customize which class to instantiate for your post, use Class Maps instead: https://timber.github.io/docs/v2/guides/class-maps/', '2.0.0');
        }
    }
    /**
    ...
    {
        return \array_filter(\get_nav_menu_locations(), fn($location) => \is_string($location) || \is_int($location));
    }
    /* Comment Retrieval
       ================================ */
    /**
    ...
    {
        if (!\defined('PicowindDeps\TIMBER_LOADED')) {
            self::init();
        }
