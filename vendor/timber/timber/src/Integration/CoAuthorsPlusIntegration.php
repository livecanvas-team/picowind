...
use PicowindDeps\CoAuthors_Plus;
use PicowindDeps\Timber\Integration\CoAuthorsPlus\CoAuthorsPlusUser;
use WP_User;
class CoAuthorsPlusIntegration implements IntegrationInterface
{
    ...
    {
        $authors = [];
        $cauthors = \PicowindDeps\get_coauthors($post->ID);
        foreach ($cauthors as $author) {
            $uid = $this->get_user_uid($author);
            if ($uid) {
                $authors[] = \PicowindDeps\Timber\Timber::get_user($uid);
            } else {
                $wp_user = new WP_User($author);
                $user = \PicowindDeps\Timber\Timber::get_user($wp_user);
