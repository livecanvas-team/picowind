...
use Closure;
use PicowindDeps\Illuminate\Bus\Events\BatchDispatched;
use PicowindDeps\Illuminate\Contracts\Container\Container;
use PicowindDeps\Illuminate\Contracts\Events\Dispatcher as EventDispatcher;
...
use PicowindDeps\Laravel\SerializableClosure\SerializableClosure;
use Throwable;
class PendingBatch
{
    use Conditionable;
    ...
    protected function store($repository)
    {
        ...
        collect($this->beforeCallbacks())->each(function ($handler) use ($batch) {
            try {
                ...
                if (function_exists('PicowindDeps\report')) {
                    report($e);
                }
            }
        });
